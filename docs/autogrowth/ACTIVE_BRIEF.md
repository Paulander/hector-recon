...
- Suggested next step: rerun edge/fence validation with the terminal-native foundation path and keep ActionRanker only as an ablation baseline. An external audit is still useful after TG26h/TG26i to judge convergence versus bloat, but do not proceed to broad KRK, ecological spawning, SCRIPT/LAG expansion, or fence M4 consolidation until edge/fence transfer is retested through the revived terminal substrate.
