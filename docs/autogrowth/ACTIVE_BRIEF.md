...
- Current TG24 result: TG18-TG23 did not use `krk_reward`, `KRK_STAGES`, `box_min_side`, or `did_box_grow` in their runtime loops. TG24 now records old curriculum reward components, excess moves where exact curriculum positions apply, box/confinement trajectories, repetition/fivefold, repeated white actions, rook attacked/missing, and non-terminal progress. Heldout h40 candidate-on remains TG20-like at 1/200 mates with no rook-loss/stalemate regressions and +0.97 graded-credit delta, but it also exposes 3 confinement regressions. Curriculum-probe slices regress from 3/19 baseline mates to 2/19 candidate-on mates at h40/h80.
- Next checkpoint should use TG24 graded credit to train or freeze one local precision gate, then confirm with M3 frozen before any M4 promotion. Do not add more ungated retry candidates.
