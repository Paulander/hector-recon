...
- Next checkpoint: precompute/cache handoff-eligible edge/fence position pools or equivalent indexed generator state, then scale the handoff-filtered curriculum before any broad KRK run. Do not consolidate M4 until larger held-out confirmation passes advancement thresholds and foundation regression stays clean.
