#!/usr/bin/env python3
"""Merge candidate-generation capacity labels into dataset v2 and rerun probe."""
...
OUT_DATASET_MD = Path(
    "reports/strategy_arbitration/krk_strategy_sequence_dataset_v2_capacity_merged.md"
)
OUT_PROBE_JSON = Path(
    "reports/strategy_arbitration/krk_candidate_generation_refresh_probe_v2_after_labels.json"
)
OUT_PROBE_MD = Path(
    "reports/strategy_arbitration/krk_candidate_generation_refresh_probe_v2_after_labels.md"
)
...
def _capacity_label(result: str | None) -> str:
    if result == "mate":
...
def label_to_row(label: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "row_id": f"cg_capacity_label_v2.{label.get('job_id')}",
        ...
        "source_terms": ["offline_candidate_generation_capacity_evidence_v2"],
        "move_shape_terms": [],
        "post_move_terms": [],
        "safety_terms": [],
...
def _summarize(rows: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    selector_counts = Counter(
        row.get("evidence_channel") for row in rows if row.get("usable_for_selector_training_v2")
    )
    stage7_count = sum(1 for row in rows if row.get("stage7_challenge_row"))
    return {
        "row_count": len(rows),
        ...
        "selector_training_row_count_by_channel": dict(sorted(selector_counts.items())),
        ...
        "selector_training_row_count": sum(
            1 for row in rows if row.get("usable_for_selector_training_v2")
        ),
...
def build_dataset_payload(
    ...
    base_rows = list(dataset.get("rows") or [])
    label_rows = [
        label_to_row(label)
        for label in labels.get("labels") or []
        if _capacity_label(label.get("result")) in {"positive_capacity", "negative_capacity"}
    ]
    rows = base_rows + label_rows
    summary = _summarize(rows)
    return {
        "schema_version": "krk_strategy_sequence_dataset.v2_capacity_merged",
        ...
        "source_artifacts": [str(DATASET), str(LABELS)],
        "summary": {
            **summary,
            "merged_label_row_count": len(label_rows),
            "merged_label_capacity_counts": dict(
                sorted(Counter(row["capacity_label"] for row in label_rows).items())
            ),
        },
        "rows": rows,
        "decision": {
...
def _write_dataset_md(payload: dict[str, Any]) -> None:
    ...
    lines = [
        ...
        f"- merged_label_row_count: {summary['merged_label_row_count']}",
        f"- merged_label_capacity_counts: `{summary['merged_label_capacity_counts']}`",
