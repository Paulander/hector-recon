#!/usr/bin/env python3
"""Probe context-enriched ownership-selection labels non-causally."""
...
DATASET = Path("reports/krk_ownership_selection_context_dataset_v0.json")
OUT_JSON = Path("reports/krk_ownership_selection_context_feature_probe_v0.json")
OUT_MD = Path("reports/krk_ownership_selection_context_feature_probe_v0.md")
...
def _term_flag(row: dict[str, Any], term: str) -> str:
    terms = set(row.get("source_terms") or []) | set(row.get("context_terms") or [])
    return str(term in terms)
...
def build_probe() -> dict[str, Any]:
    ...
    rows = [row for row in dataset.get("rows") or [] if row.get("source_stage") != "stage7"]
    specs = {
        "provider_family": ("provider_family",),
        "stage_provider_family": ("source_stage", "provider_family"),
        "raw_score_bucket": ("raw_score_bucket",),
        "provider_stage_raw": ("provider_family", "source_stage", "raw_score_bucket"),
        "provider_edge_support": (
            "provider_family",
        ...
        "provider_edge_piece": (
            "provider_family",
            "ctx:terminal_space_context.black_king_edge_bucket",
            "ctx:selected_move_context.selected_piece",
        ),
        "provider_move_deltas": (
            "provider_family",
            "ctx:selected_move_context.selected_piece",
            "ctx:selected_move_context.king_distance_delta_bucket",
            "ctx:selected_move_context.box_area_delta_bucket",
        ),
        "provider_context_compact": (
            "provider_family",
            "source_stage",
            "ctx:terminal_space_context.black_king_edge_bucket",
            "ctx:terminal_space_context.white_king_support_bucket",
        ...
        "provider_context_full": (
            ...
            "raw_score_bucket",
            "ctx:terminal_space_context.black_king_edge_bucket",
            "ctx:terminal_space_context.box_area_relevance",
            "ctx:terminal_space_context.white_king_support_bucket",
            "ctx:selected_move_context.selected_piece",
            "ctx:selected_move_context.king_distance_delta_bucket",
            "ctx:selected_move_context.rook_distance_delta_bucket",
            "ctx:selected_move_context.box_area_delta_bucket",
        ),
    }
    thresholds = (0.45, 0.5, 0.6, 0.75)
    results = {
    ...
    balanced_candidates = [
        ...
        if (result.get("positive_recall") or 0.0) >= 0.7
    ]
    ...
    underpowered = len(rows) < 40 or sum(1 for row in rows if not _positive(row)) < 12
    status = "context_features_underpowered"
    if (best.get("negative_suppression") or 0) >= 0.6 and (best.get("positive_recall") or 0) >= 0.7:
        status = "context_features_promising_for_architecture_review"
    payload = {
        "schema_version": "krk_ownership_selection_context_feature_probe.v0",
...
def render_markdown(payload: dict[str, Any]) -> str:
    ...
    lines.extend(
        [
            ...
            "",
            f"`{payload['best_balanced_result']}`",
            "",
            "## Decision",
            "",
        ]
