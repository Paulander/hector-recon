#!/usr/bin/env python3
"""Fold approved candidate-generation refresh sandbox frames into trace features."""
...
def _refresh_frames(case: dict[str, Any]) -> list[dict[str, Any]]:
    return [
        frame
...
def trace_frames(sandbox: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    for case_idx, case in enumerate(sandbox.get("cases") or []):
        ...
        if case.get("source_stage") not in {"stage5", "stage6"}:
            continue
        state_id = str(case.get("state_id") or f"candidate_generation_refresh_state_{case_idx}")
        for frame_idx, frame in enumerate(_refresh_frames(case)):
            frames.append(
                {
                    "schema_version": "krk_strategy_sequence_candidate_frame.v1",
                    "frame_id": (
                        f"ssf.runtime_observation.candidate_generation_refresh."
                    ...
                    "candidate_id": (
                        f"candidate.generation_refresh.{frame.get('provider_id')}.{frame.get('move_id')}"
                    ),
...
def build_payload(
    *,
    ...
    return {
        "schema_version": "krk_strategy_sequence_candidate_generation_refresh_trace_features.v1",
        ...
        **_runtime_false_block(),
        ...
        "decision": {
            "status": (
                ...
                if trace_only_safe
                else "candidate_generation_refresh_trace_feature_fold_blocked"
            ),
            "selector_allowed": False,
            "guardrails_allowed": False,
            "promotion_allowed": False,
            "recommended_next_step": (
                "build_strategy_sequence_dataset_v4_non_causal"
                if trace_only_safe
                else "quarantine_candidate_generation_refresh_trace_features"
            ),
        },
    }
