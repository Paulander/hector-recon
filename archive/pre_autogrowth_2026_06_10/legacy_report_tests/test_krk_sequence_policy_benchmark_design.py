...
def test_sequence_policy_benchmark_design_is_ready_but_blocks_training_and_runtime():
    ...
    assert (
        ...
        == "sequence_policy_benchmark_mixed_plan_window_underpowered_blocked_pending_"
    ...
    assert payload["readiness"]["current_benchmark_review_available"] is False
    assert (
        payload["readiness"][
    ...
    assert (
        ...
        == "implement_non_causal_sequence_policy_benchmark"
    )
    passive = payload["passive_design_without_new_labels"]
    assert passive["status"] == "non_causal_sequence_policy_design_review_needed"
    ...
    assert passive["current_evidence_limit"] is None
...
def test_sequence_policy_benchmark_design_routes_missing_collection_command_to_gate_review():
    ...
    payload = _design.build_payload(benchmark_review=benchmark_review)

    assert (
        payload["decision"]["status"]
        == "sequence_policy_benchmark_design_blocked_pending_"
        "protected_failure_contrast_control_plane_gate_review"
    )
    assert (
        payload["decision"]["recommended_next_step"]
