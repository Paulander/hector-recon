...
- `krk.stage0_basin` selected 9 times
...
- `krk.box_shrink` via `krk.stage0_basin` resulted in `max_plies` (count=9, gap=False, conflict=False)

Selected successor by outcome:
- `krk.edge_trap_close:mate`: 10
- `krk.stage0_basin:max_plies`: 9
...
- `reward_contract_mismatch`: {'max_plies': 9}
...
- `reward=true|visible_fence=false|fence_survived_reply=false|conversion=max_plies`: 9
