#!/usr/bin/env python3
"""Foreground-only systemd coordinator for one carried-slot reclosure.

This transport program imports no ReCoN module.  The systemd service owns this
process directly; it verifies the immutable completed slot 01, then runs only
the remaining frozen commands synchronously and never retries.
"""
...
def load_manifest(path: Path, expected_sha256: str) -> dict[str, Any]:
    ...
    if manifest["schema_version"] != "native_v2_systemd_schema_reclosure.v1":
...
def require_exact_json(value: Any, expected: Any, *, label: str) -> None:
    """Require exact JSON keys, values, and JSON scalar types."""

    if canonical_bytes(value) != canonical_bytes(expected):
        raise RuntimeError(f"{label} changed")


def verify_child_result_outcome_access(value: Any, *, label: str) -> None:
    require_exact_json(value, CHILD_RESULT_ZERO_OUTCOME, label=label)


def verify_transport_outcome_access(value: Any, *, label: str) -> None:
    require_exact_json(value, ZERO_OUTCOME, label=label)


def verify_carried_slot(root: Path, manifest: Mapping[str, Any]) -> dict[str, Any]:
    carried = manifest["carried_slot"]
    child_slot = carried["child_slot"]
    attempt_root = root / manifest["child_attempt_root"]
    attempt = attempt_root / child_slot
    expected_names = set(carried["file_sha256"])
    if not attempt.is_dir():
        raise RuntimeError(f"carried child attempt directory absent:{child_slot}")
    actual_names = {path.name for path in attempt.iterdir()}
    if actual_names != expected_names:
        raise RuntimeError(
            f"carried child directory shape mismatch:{child_slot}:"
            f"{sorted(actual_names)}"
        )
    for name, expected in carried["file_sha256"].items():
        observed = sha256_file(attempt / name)
        if observed != expected:
            raise RuntimeError(
                f"carried child file changed:{child_slot}:{name}:{observed}"
            )
        committed = sha256_bytes(
            git_bytes(
                root,
                f"{carried['source_commit']}:"
                f"{manifest['child_attempt_root']}/{child_slot}/{name}",
            )
        )
        if committed != expected:
            raise RuntimeError(
                f"carried child source binding changed:{child_slot}:{name}"
            )

    result = load_json(attempt / "result.json")
    progress = load_json(attempt / "progress.json")
    if result.get("record_digest") != carried["result_record_digest"]:
        raise RuntimeError("carried child result record digest changed")
    if result.get("stage") != "passed":
        raise RuntimeError("carried child result is not passed")
    if progress.get("completed_unit_count") != 96:
        raise RuntimeError("carried child progress is incomplete")
    verify_child_result_outcome_access(
        result.get("outcome_access"), label="carried child outcome access"
    )

    captured = carried["captured_records"]
    captured_values: dict[str, dict[str, Any]] = {}
    for name, binding in captured.items():
        raw = git_bytes(root, f"{carried['source_commit']}:{binding['path']}")
        if sha256_bytes(raw) != binding["sha256"]:
            raise RuntimeError(f"carried captured record changed:{name}")
        if binding.get("json", False):
            value = json.loads(raw.decode("utf-8"))
            if not isinstance(value, dict):
                raise RuntimeError(f"carried captured JSON is not object:{name}")
            captured_values[name] = value
    terminal = captured_values["child_terminal"]
    if terminal.get("returncode") != 0 or terminal.get("signal") is not None:
        raise RuntimeError("carried child terminal status changed")
    if terminal.get("pid") != carried["historical_child_pid"]:
        raise RuntimeError("carried child PID changed")
    if terminal.get("record_digest") != carried["child_terminal_record_digest"]:
        raise RuntimeError("carried child terminal digest changed")
    if terminal.get("stdout_sha256") != carried["file_sha256"]["result.json"]:
        raise RuntimeError("carried child stdout/result identity changed")
    if terminal.get("stderr_size") != 0:
        raise RuntimeError("carried child stderr is not empty")

    for entry in manifest["executions"]:
        slot = entry["child_slot"]
        if slot != child_slot and (attempt_root / slot).exists():
            raise RuntimeError(f"future child attempt already exists:{slot}")
    return {
        "child_slot": child_slot,
        "historical_child_pid": carried["historical_child_pid"],
        "result_sha256": carried["file_sha256"]["result.json"],
        "result_record_digest": carried["result_record_digest"],
        "child_terminal_record_digest": carried["child_terminal_record_digest"],
        "outcome_access": dict(ZERO_OUTCOME),
    }


def frozen_task_plan(manifest: Mapping[str, Any]) -> list[dict[str, str]]:
    carried_slot = manifest["carried_slot"]["child_slot"]
    rows: list[dict[str, str]] = []
    for entry in manifest["executions"]:
        slot = entry["child_slot"]
        sequence = entry["sequence"]
        if slot != carried_slot:
            rows.append(
                {
                    "task": f"slot-{sequence:02d}-child",
                    "kind": "child",
                    "child_slot": slot,
                }
            )
        rows.append(
            {
                "task": f"slot-{sequence:02d}-verifier",
                "kind": "verifier",
                "child_slot": slot,
            }
        )
...
def complete_preflight(
    ...
    carried = verify_carried_slot(root, manifest)
    verify_untracked_outputs(root, manifest, [carried["child_slot"]])
    if frozen_task_plan(manifest) != manifest["task_plan"]:
        raise RuntimeError("frozen task plan changed")
    service_contract = (
    ...
    return {
        ...
        "carried_slot": carried,
        "task_plan": frozen_task_plan(manifest),
        "outcome_access": dict(ZERO_OUTCOME),
    }
...
def verify_child_directory(
    ...
    verify_child_result_outcome_access(
        result.get("outcome_access"),
        label=f"child result outcome access:{child_slot}",
    )
...
def load_successful_json_stdout(path: Path, *, label: str) -> dict[str, Any]:
    ...
    verify_transport_outcome_access(
        value.get("outcome_access"), label=f"{label} outcome access"
    )
    return value
...
def series_terminal(
    ...
    *,
    ...
    value: dict[str, Any] = {
        "schema_version": "native_v2_systemd_schema_reclosure_terminal.v1",
        "recovery_series_id": manifest["recovery_series_id"],
...
def run_series(manifest_path: Path, manifest_sha256: str) -> int:
    ...
    carried_slot = manifest["carried_slot"]["child_slot"]
    used_pids: set[int] = {manifest["carried_slot"]["historical_child_pid"]}
    completed_slots: list[str] = [carried_slot]
    ...
    try:
        ...
        carried_entry = next(
            entry
            for entry in manifest["executions"]
            if entry["child_slot"] == carried_slot
        )
        carried_result = verify_child_directory(root, manifest, carried_slot)
        carried_result.update(
            {
                "historical_child_pid": manifest["carried_slot"][
                    "historical_child_pid"
                ],
                "source_commit": manifest["carried_slot"]["source_commit"],
            }
        )
        carried_result["record_digest"] = digest(carried_result)
        atomic_write(record_root / "slot-01-carried-child-check.json", carried_result)
        verify_after_task(root, manifest, completed_slots)

        verifier_name = "slot-01-verifier"
        verifier_terminal = run_command(
            command=carried_entry["verifier_command"],
            cwd=root,
        ...
        if verifier_terminal["returncode"] != 0:
            raise RuntimeError(f"verifier stopped nonzero:{carried_slot}")
        ...
        if verifier_value.get("attempt_id") != carried_slot:
        ...
        terminal = series_terminal(
            manifest,
            classification=(
                "Carried-slot portable admission and exact historical "
...
def finalize(manifest_path: Path, manifest_sha256: str) -> dict[str, Any]:
    ...
    value = {
        "schema_version": "native_v2_systemd_schema_reclosure_finalization.v1",
        "recovery_series_id": manifest["recovery_series_id"],
        "unit": manifest["systemd"]["unit_name"],
