...
def test_tg29d_smoke_reports_low_progress_no_cheat_fields(tmp_path: Path) -> None:
    result = run_online_low_progress_repair(
        config=OnlineLowProgressRepairConfig(
            ...
            repair_arms=("combined_reply_robust", "balanced_reply_robust_plus_progress"),
        )
    )

    payload = result.to_dict()
    decision = payload["decision"]
    assert payload["checkpoint"] == "TG29d_online_low_progress_repair"
    ...
    assert "low_progress_diagnostics" in payload
