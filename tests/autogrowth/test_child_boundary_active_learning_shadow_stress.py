...
def test_tg32_active_learning_shadow_stress_tier1_shadow_only(tmp_path: Path) -> None:
    result = run_child_boundary_active_learning_shadow_stress(
        config=ChildBoundaryActiveLearningShadowStressConfig(
            ...
            active_boundary_pool_path=str(tmp_path / "active.jsonl"),
            active_boundary_pool_index_path=str(tmp_path / "active_index.json"),
            child_coverage_pool_path=str(tmp_path / "child.jsonl"),
            child_coverage_pool_index_path=str(tmp_path / "child_index.json"),
            child_arm_results_path=str(tmp_path / "arms.jsonl"),
            hard_decoy_pool_path=str(tmp_path / "hard_decoys.jsonl"),
            hard_decoy_pool_index_path=str(tmp_path / "hard_decoys_index.json"),
            shadow_online_artifact_path=str(tmp_path / "shadow.json"),
            seed_count=2,
            cycles_per_arm=250,
            max_cycles_per_arm=250,
            target_tier=1,
        )
    )
    output = result.write_json(tmp_path / "tg32.json")
    decision = result.decision
    assert output.exists()
    assert Path(result.config.base.progress_output).exists()
    assert Path(result.config.active_boundary_pool_path).exists()
    assert Path(result.config.active_boundary_pool_index_path).exists()
    assert Path(result.config.child_coverage_pool_path).exists()
    assert Path(result.config.child_coverage_pool_index_path).exists()
    assert Path(result.config.child_arm_results_path).exists()
    assert Path(result.config.hard_decoy_pool_path).exists()
    assert Path(result.config.hard_decoy_pool_index_path).exists()
    assert Path(result.config.shadow_online_artifact_path).exists()
    ...
    required = {
        ...
        "parent_hard_decoy_false_positive_count",
        "child_arm_count",
        "child_seed_count",
        "cycles_per_arm",
        "max_cycles_per_arm",
        "selected_child_arm",
        "selected_child_arm_reason",
        ...
        "child_hard_decoy_recognized_count",
        "child_boundary_coverage_rate",
        "child_heldout_boundary_coverage_rate",
        "child_regression_boundary_coverage_rate",
        "child_train_heldout_gap",
        "child_heldout_regression_gap",
        "child_worst_seed_heldout_coverage_rate",
        "child_mean_seed_heldout_coverage_rate",
        "child_median_seed_heldout_coverage_rate",
        "child_std_seed_heldout_coverage_rate",
        ...
        "evidence_family_seed_stability_by_arm",
        "evidence_family_train_heldout_gap_by_arm",
        "evidence_family_hard_decoy_effect_by_arm",
        "shadow_child_used",
        "shadow_child_used_in_main_eval",
        "parent_main_targeted_success_count",
        "child_shadow_targeted_success_count",
        "child_shadow_targeted_success_rate",
        ...
        "child_shadow_hard_decoy_false_handoff_count",
        "child_shadow_success_by_start_set",
        "child_shadow_success_by_reply_policy",
        "child_shadow_success_by_horizon",
        "child_boundary_recognition_online_success_correlation",
    ...
    assert decision["expanded_boundary_pool_entry_count"] == 1024
    assert decision["unique_boundary_fen_count"] == 1024
    assert decision["split_group_leak_count"] == 0
    assert decision["active_learning_round_count"] > 0
    assert decision["hard_decoy_count"] == 96
    assert decision["child_confusable_decoy_count"] == 96
    assert decision["child_heldout_recognized_count"] > 0
    assert decision["child_regression_recognized_count"] > 0
    assert decision["child_decoy_false_handoff_count"] == 0
    assert decision["child_hard_decoy_false_positive_count"] == 0
    ...
    rows = [
        json.loads(line)
        for line in Path(result.config.active_boundary_pool_path).read_text().splitlines()
        if line.strip()
    ]
    assert len(rows) == 1024
    assert len({row["canonical_fen"] for row in rows}) == 1024
