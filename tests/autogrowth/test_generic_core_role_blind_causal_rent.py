...
RUNNER = runpy.run_path(
    str(ROOT / "scripts/autogrowth/run_generic_core_role_blind_causal_rent.py")
)
...
def test_capacity_configuration_is_identical_for_every_action_channel() -> None:
    ...
    RUNNER["_configure_arm"](rent, "causal_rent_ranked")
    ...
    assert rent.causal_rent_config is not None
    assert all(
        channel.learner.external_lifecycle
        for channel in rent.channels.values()
    )
...
def test_common_experience_shadows_receive_identical_records() -> None:
    ...
    RUNNER["_configure_arm"](ranked, "causal_rent_ranked")
    RUNNER["_configure_arm"](shuffled, "causal_rent_shuffled")
    stream = [
        {
