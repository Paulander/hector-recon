...
RUNNER = runpy.run_path(
    str(ROOT / "scripts/autogrowth/run_generic_core_support_conditioned_lifecycle_grace.py")
)
...
def test_only_request_and_lifecycle_modes_vary_across_rent_arms() -> None:
    policy = EpisodicCompositionPolicy(
        ("action_a", "action_b"),
        random_seed=81,
        reservoir_config=ExperienceReservoirConfig(capacity=64),
    ...
    assert {
        arm: config.lifecycle_grace_mode
        for arm, config in configs.items()
    } == {
    ...
    assert len({
        tuple(
            ...
            if name not in {
                "exploration_request_mode", "lifecycle_grace_mode"
            }
        )
...
def test_target_diagnostics_reconstruct_exact_four_post_hoc() -> None:
    ...
    for action_id in task["door_action_ids"]:
        ...
        channel.sync_external_lifecycle()
    RUNNER["_configure_arm"](
        policy, "rent_batch_4_graph_exact_fixed_six"
    )
...
def test_fresh_admission_protocol_constants_are_frozen() -> None:
    assert RUNNER["CANDIDATE_SEEDS"] == tuple(range(20262401, 20262441))
...
def test_planted_priority_canary_is_identifiable_and_rng_matched() -> None:
    canary = RUNNER["_priority_canary"]()

    assert canary["pass"] is True
    assert canary["instrumentation_only"] is True
    assert canary["multi_request_fraction"] >= 0.40
    assert canary["unequal_strength_fraction"] >= 0.30
    assert canary["selected_action_divergence_fraction"] >= 0.20
    assert canary["allocator_could_differ_count"] == 100
    assert canary["event_and_rng_budget_parity"] is True
    assert canary["directed_action_sha256"] != canary["shuffled_action_sha256"]
