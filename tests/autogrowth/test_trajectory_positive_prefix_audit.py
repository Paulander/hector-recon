...
def test_tg29g_smoke_writes_pool_and_no_cheat_fields(tmp_path: Path) -> None:
    ...
    output = result.write_json(tmp_path / "tg29g.json")
    payload = result.to_dict()
    decision = payload["decision"]
    assert output.exists()
    assert Path(payload["pool_index"]["trajectory_pool_path"]).exists()
    assert Path(payload["pool_index"]["trajectory_pool_index_path"]).exists()
    assert payload["checkpoint"] == "TG29g_trajectory_positive_prefix_audit"
