...
def test_tg29c_smoke_reports_reply_robust_no_cheat_fields(tmp_path: Path) -> None:
    result = run_reply_robust_bridge_pressure(
        config=ReplyRobustBridgePressureConfig(
            ...
            repair_arms=("baseline_no_repair", "combined_reply_robust"),
            max_reply_envelope_replies_per_candidate=2,
        )
    )

    payload = result.to_dict()
    decision = payload["decision"]
    assert payload["checkpoint"] == "TG29c_reply_robust_bridge_pressure"
    ...
    assert "success_rate_by_black_reply_policy" in decision
    assert "bridge_loop_diagnostics" in payload
