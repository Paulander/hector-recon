...
def test_tg31_scales_boundary_child_ladder_shadow_only(tmp_path: Path) -> None:
    result = run_child_boundary_coverage_scale_shadow_stability(
        config=ChildBoundaryCoverageScaleShadowStabilityConfig(
            ...
            scaled_boundary_pool_path=str(tmp_path / "scaled.jsonl"),
            scaled_boundary_pool_index_path=str(tmp_path / "scaled_index.json"),
    ...
    output = result.write_json(tmp_path / "tg31.json")
    decision = result.decision
    assert output.exists()
    assert Path(result.config.base.progress_output).exists()
    assert Path(result.config.scaled_boundary_pool_path).exists()
    assert Path(result.config.scaled_boundary_pool_index_path).exists()
    ...
    assert decision["expanded_boundary_pool_entry_count"] == 256
    assert decision["unique_boundary_fen_count"] == 256
    assert decision["lineage_group_count"] == 256
    assert decision["split_group_leak_count"] == 0
    assert decision["boundary_train_count"] == 96
    assert decision["boundary_heldout_count"] == 64
    assert decision["boundary_regression_count"] == 48
    assert decision["boundary_decoy_count"] == 48
    assert decision["child_arm_count"] >= 10
    assert decision["child_seed_count"] == 2
    ...
    rows = [
        json.loads(line)
        for line in Path(result.config.scaled_boundary_pool_path).read_text().splitlines()
        if line.strip()
    ]
    assert len(rows) == 256
    assert len({row["canonical_fen"] for row in rows}) == 256
