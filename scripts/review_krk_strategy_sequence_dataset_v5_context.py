#!/usr/bin/env python3
"""Review KRK strategy-sequence dataset v5 context integration."""
...
OUT_JSON = Path("reports/strategy_arbitration/krk_strategy_sequence_dataset_v5_context_review.json")
OUT_MD = Path("reports/strategy_arbitration/krk_strategy_sequence_dataset_v5_context_review.md")
...
def build_payload(
    ...
    context_ready = (
        ...
        and trace_sources.get("exact_trace_enrichment_sandbox", 0) > 0
        and trace_sources.get("repair_monitor_observation", 0) > 0
    )
    return {
        "schema_version": "krk_strategy_sequence_dataset_v5_context_review.v1",
        ...
        "summary": {
            ...
            "exact_trace_enrichment_trace_row_count": qs.get(
        ...
        "decision": {
            "status": (
                ...
                if context_ready
                else "strategy_sequence_dataset_v5_context_blocked"
            ),
            "selector_allowed": False,
            "guardrails_allowed": False,
            "promotion_allowed": False,
            "recommended_next_step": (
                "candidate_generation_v5_context_benchmark_non_causal"
                if context_ready
                else "fix_dataset_v5_context_integration"
            ),
        },
    }
