#!/usr/bin/env python3
"""Probe KRK strategy-sequence dataset v4 quality without enabling selection."""
...
OUT_JSON = Path("reports/strategy_arbitration/krk_strategy_sequence_dataset_v4_quality_probe.json")
OUT_MD = Path("reports/strategy_arbitration/krk_strategy_sequence_dataset_v4_quality_probe.md")
...
def build_payload(dataset: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    generator_rows = [
        row for row in rows if row.get("usable_for_candidate_generation_training_v4")
    ]
    selector_rows = [row for row in rows if row.get("usable_for_selector_training_v4")]
    trace_rows = [
    ...
    quality_checks = {
        ...
        "refresh_trace_rows_are_context_only": all(
            not row.get("usable_for_selector_training_v4")
            and not row.get("usable_for_candidate_generation_training_v4")
    ...
    status = (
        ...
        else "strategy_sequence_dataset_v4_quality_blocked"
    )
    return {
        "schema_version": "krk_strategy_sequence_dataset_v4_quality_probe.v1",
        ...
        "interpretation": {
            ...
            "reason": (
                "V4 integrates approved candidate-generation refresh sandbox traces "
                "as observation context and preserves positive capacity rows for "
                "candidate-generation recall, but still has no explicit ownership "
                "selector labels."
            ),
        },
        "decision": {
            ...
            "recommended_next_step": (
                ...
                else "fix_dataset_v4_channel_semantics"
            ),
        },
    }
...
def write_markdown(payload: dict[str, Any]) -> None:
    ...
    lines = [
        "# KRK Strategy-Sequence Dataset v4 Quality Probe",
        "",
        "This probe validates dataset v4 channel semantics. It does not train a selector or authorize runtime changes.",
        "",
        "## Decision",
        "",
