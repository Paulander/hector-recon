#!/usr/bin/env python3
"""Build context dataset from targeted-refreshed ownership labels."""
...
OWNERSHIP = Path("reports/krk_ownership_selection_label_dataset_v4.json")
TARGETED_LABELS = Path("reports/krk_targeted_non_stage0_ownership_labels_v0.json")
OUT_JSON = Path("reports/krk_ownership_selection_context_dataset_v2.json")
OUT_MD = Path("reports/krk_ownership_selection_context_dataset_v2.md")
...
def build_dataset() -> dict:
    ...
    payload["schema_version"] = "krk_ownership_selection_context_dataset.v2"
    payload["source_artifacts"][0] = str(OWNERSHIP)
    payload["decision"]["recommended_next_step"] = (
        "probe_targeted_refreshed_context_enriched_ownership_selection_features"
    )
    return payload
...
def render_markdown(payload: dict) -> str:
    return context_v0.render_markdown(payload).replace(
        ...
        "# KRK Ownership Selection Context Dataset v2",
        1,
    )
