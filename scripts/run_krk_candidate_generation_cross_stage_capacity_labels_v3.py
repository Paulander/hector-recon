#!/usr/bin/env python3
"""Run bounded cross-stage candidate-generation capacity labels v3."""
...
from run_krk_candidate_generation_capacity_evidence_labels_v2 import (  # noqa: E402
    EXECUTION_BINDING,
...
def run_labels(manifest: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    for index, job in enumerate(jobs, start=1):
        print(
            f"[{index}/{len(jobs)}] {job.get('stage_family_cell')} {job.get('provider_id')}",
            file=sys.stderr,
            flush=True,
        )
        label = _compact_label(forced_labels._run_job(ROOT, _execution_job(job), cache))
        label["label_channel"] = "candidate_generation_cross_stage_capacity_v3"
        ...
        label["stage_family_cell"] = job.get("stage_family_cell")
        label["target_cell_maturity"] = job.get("target_cell_maturity")
        label["stage7_training_row"] = False
        labels.append(label)
    result_counts = Counter(str(label.get("result") or "unknown") for label in labels)
    by_cell = Counter(
        f"{label.get('stage_family_cell')}:{label.get('result')}" for label in labels
    )
    payload = {
        "schema_version": "krk_candidate_generation_cross_stage_capacity_labels.v3",
        ...
        "summary": {
            ...
            "result_counts_by_stage_family_cell": dict(sorted(by_cell.items())),
...
def write_markdown(payload: dict[str, Any]) -> None:
    ...
    for label in payload["labels"]:
        lines.append(
            f"- `{label['job_id']}` cell=`{label.get('stage_family_cell')}` "
