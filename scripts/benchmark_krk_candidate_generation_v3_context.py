#!/usr/bin/env python3
"""Benchmark dataset v3 candidate-generation context coverage non-causally."""
...
QUALITY = Path("reports/strategy_arbitration/krk_strategy_sequence_dataset_v3_quality_probe.json")
OUT_JSON = Path("reports/strategy_arbitration/krk_candidate_generation_v3_context_benchmark.json")
OUT_MD = Path("reports/strategy_arbitration/krk_candidate_generation_v3_context_benchmark.md")
...
def build_payload(
    ...
    return {
        ...
        "summary": {
            ...
            "state_provider_positive_capacity_recall_from_trace": state_provider_recall,
            "stage_family_positive_capacity_recall_from_trace": stage_family_recall,
            "stage_family_negative_capacity_exposure_from_trace": negative_stage_family_exposure,
            "exact_positive_capacity_covered_count": len(exact_positive),
            "state_provider_positive_capacity_covered_count": len(state_provider_positive),
            "stage_family_positive_capacity_covered_count": len(stage_family_positive),
            "stage_family_negative_capacity_exposed_count": len(stage_family_negative),
        ...
        "interpretation": {
            ...
            "risk": (
                "Trace context improves visibility of Stage 5/6 proposal scope, "
                "but stage-family coverage also overlaps negative-capacity families; "
                "it cannot become a selector without explicit ownership labels."
            ),
        },
        "decision": {
            "status": (
                ...
                if context_useful
                else "candidate_generation_v3_context_underpowered"
            ),
            "selector_allowed": False,
            "guardrails_allowed": False,
            "promotion_allowed": False,
            "recommended_next_step": (
                "architecture_review_candidate_generation_context_to_runtime_boundary"
                if context_useful
...
def write_markdown(payload: dict[str, Any]) -> None:
    ...
    lines.extend(
        [
            ...
            "## Interpretation",
            "",
            f"- context_useful_for_candidate_generation_analysis: `{payload['interpretation']['context_useful_for_candidate_generation_analysis']}`",
            f"- selector_supported: `{payload['interpretation']['selector_supported']}`",
            f"- risk: {payload['interpretation']['risk']}",
        ]
    )
