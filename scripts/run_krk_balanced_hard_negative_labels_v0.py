#!/usr/bin/env python3
"""Run bounded balanced protected hard-negative labels."""
...
REVIEW = Path("reports/krk_balanced_hard_negative_execution_manifest_review_v0.json")
OUT_JSON = Path("reports/krk_balanced_hard_negative_labels_v0.json")
OUT_MD = Path("reports/krk_balanced_hard_negative_labels_v0.md")
...
def _capacity_label(result: Any) -> str:
    if result == "mate":
...
def run_labels() -> dict[str, Any]:
    ...
    for index, job in enumerate(jobs, start=1):
        ...
        label["label_channel"] = "balanced_protected_hard_negative_capacity"
        ...
        label["fen"] = job.get("fen")
        label["capacity_label"] = _capacity_label(label.get("result"))
    ...
    by_provider = Counter(f"{label.get('provider_family')}:{label.get('result')}" for label in labels)
    payload = {
        "schema_version": "krk_balanced_hard_negative_labels.v0",
        ...
        "summary": {
            ...
            "result_counts_by_provider_family": dict(sorted(by_provider.items())),
            ...
            "negative_capacity_count": sum(1 for label in labels if label.get("capacity_label") == "negative_capacity"),
            "positive_capacity_count": sum(1 for label in labels if label.get("capacity_label") == "positive_capacity"),
...
def render_markdown(payload: dict[str, Any]) -> str:
    ...
    for label in payload["labels"]:
        lines.append(
            ...
            f"result=`{label['result']}` capacity=`{label['capacity_label']}` "
            f"plies=`{label['plies']}` forced_move=`{label.get('forced_first_move')}`"
        )
