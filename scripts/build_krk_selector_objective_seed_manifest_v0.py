#!/usr/bin/env python3
"""Build a non-causal selector-objective seed manifest from joined trace/ownership evidence."""

from __future__ import annotations

import json
from collections import Counter
from pathlib import Path
from typing import Any


ROOT = Path(__file__).resolve().parents[1]
REVIEW = Path("reports/strategy_arbitration/krk_ownership_label_recovery_review_v0.json")
OUT_JSON = Path("reports/strategy_arbitration/krk_selector_objective_seed_manifest_v0.json")
OUT_MD = Path("reports/strategy_arbitration/krk_selector_objective_seed_manifest_v0.md")


def _load(path: Path = REVIEW) -> dict[str, Any]:
    payload = json.loads((ROOT / path).read_text(encoding="utf-8"))
    if not isinstance(payload, dict):
        raise ValueError(f"expected JSON object: {path}")
    return payload


def _objective_channel(recovery_class: str) -> str:
    if recovery_class == "selected_failure_with_visible_positive_alternative":
        return "candidate_switch_contrast_seed"
    if recovery_class == "safe_preservation_with_visible_positive_alternative":
        return "safe_preservation_contrast_seed"
    if recovery_class == "selected_failure_with_trace_context_only":
        return "failure_context_without_candidate_seed"
    return "context_only_preservation_seed"


def build_payload(review: dict[str, Any] | None = None) -> dict[str, Any]:
    review = review or _load()
    records = [r for r in review.get("joined_records") or [] if isinstance(r, dict)]
    seed_rows = []
    for record in records:
        recovery_class = str(record.get("recovery_class") or "unknown")
        seed_rows.append(
            {
                "schema_version": "krk_selector_objective_seed_manifest_row.v0",
                "causal_status": "non_causal_selector_objective_seed",
                "state_id": record.get("state_id"),
                "source_stage": record.get("source_stage"),
                "selected_provider": record.get("selected_provider"),
                "selected_provider_family": record.get("selected_provider_family"),
                "selected_owner_label": record.get("target_label"),
                "trace_provider_candidate_count": record.get("trace_provider_candidate_count"),
                "positive_trace_provider_candidate_count": record.get(
                    "positive_trace_provider_candidate_count"
                ),
                "trace_sources": list(record.get("trace_sources") or []),
                "recovery_class": recovery_class,
                "objective_channel": _objective_channel(recovery_class),
                "usable_for_selector_training": False,
                "usable_for_runtime": False,
                "stage7_training_row": False,
            }
        )

    channel_counts = Counter(str(row.get("objective_channel")) for row in seed_rows)
    recovery_counts = Counter(str(row.get("recovery_class")) for row in seed_rows)
    has_switch = channel_counts["candidate_switch_contrast_seed"] > 0
    has_preservation = channel_counts["safe_preservation_contrast_seed"] > 0
    ready = (
        has_switch
        and has_preservation
        and (review.get("decision") or {}).get("selector_allowed") is False
        and (review.get("summary") or {}).get("stage7_row_count") == 0
    )
    return {
        "schema_version": "krk_selector_objective_seed_manifest.v0",
        "causal_status": "non_causal_selector_objective_seed_manifest",
        "runtime_behavior_changed": False,
        "runtime_defaults_changed": False,
        "runtime_selector_implemented": False,
        "runtime_score_changes": False,
        "runtime_direct_routing": False,
        "runtime_dtm_or_tablebase_lookup": False,
        "gameplay_topology_mutation": False,
        "stage7_promotion_allowed": False,
        "stage8_training_allowed": False,
        "source_artifacts": [str(REVIEW)],
        "summary": {
            "seed_row_count": len(seed_rows),
            "objective_channel_counts": dict(sorted(channel_counts.items())),
            "recovery_class_counts": dict(sorted(recovery_counts.items())),
            "candidate_switch_contrast_seed_count": channel_counts[
                "candidate_switch_contrast_seed"
            ],
            "safe_preservation_contrast_seed_count": channel_counts[
                "safe_preservation_contrast_seed"
            ],
            "selector_training_row_count": sum(
                1 for row in seed_rows if row.get("usable_for_selector_training")
            ),
            "stage7_training_row_count": sum(
                1 for row in seed_rows if row.get("stage7_training_row")
            ),
        },
        "seed_rows": seed_rows,
        "decision": {
            "status": (
                "selector_objective_seed_manifest_ready_non_causal"
                if ready
                else "selector_objective_seed_manifest_underpowered"
            ),
            "selector_allowed": False,
            "selector_training_allowed": False,
            "runtime_changes_allowed": False,
            "recommended_next_step": (
                "design_selector_objective_probe_from_seed_manifest"
                if ready
                else "collect_more_joined_trace_ownership_evidence_non_causal"
            ),
        },
    }


def write_markdown(payload: dict[str, Any]) -> None:
    lines = [
        "# KRK Selector Objective Seed Manifest v0",
        "",
        "This manifest converts joined ownership-label and trace-context evidence into seed rows for future non-causal selector-objective review. It is not selector training data.",
        "",
        "## Decision",
        "",
        f"- status: `{payload['decision']['status']}`",
        f"- selector_allowed: `{payload['decision']['selector_allowed']}`",
        f"- runtime_changes_allowed: `{payload['decision']['runtime_changes_allowed']}`",
        f"- recommended_next_step: `{payload['decision']['recommended_next_step']}`",
        "",
        "## Summary",
        "",
    ]
    for key, value in payload["summary"].items():
        lines.append(f"- {key}: `{value}`")
    lines.extend(["", "## Seed Rows", ""])
    for row in payload["seed_rows"]:
        lines.append(
            "- "
            f"`{row['state_id']}` "
            f"channel=`{row['objective_channel']}` "
            f"selected={row['selected_provider']} "
            f"label={row['selected_owner_label']} "
            f"positive_trace_candidates={row['positive_trace_provider_candidate_count']}"
        )
    (ROOT / OUT_MD).write_text("\n".join(lines) + "\n", encoding="utf-8")


def main() -> None:
    payload = build_payload()
    (ROOT / OUT_JSON).write_text(
        json.dumps(payload, indent=2, sort_keys=True) + "\n",
        encoding="utf-8",
    )
    write_markdown(payload)
    print(json.dumps(payload["decision"], indent=2, sort_keys=True))


if __name__ == "__main__":
    main()
