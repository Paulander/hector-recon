#!/usr/bin/env python3
"""Review the next runtime boundary after the v4 candidate-generation context benchmark."""
...
def build_payload(benchmark: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    exact_recall = _num(summary.get("exact_positive_capacity_recall_from_refresh_trace"))
    policy_cell_recall = _num(
        summary.get("policy_cell_positive_capacity_recall_from_refresh_trace")
    )
    policy_cell_negative_exposure = _num(
        summary.get("policy_cell_negative_capacity_exposure_from_refresh_trace")
    )
    exact_negative_exposure = _num(
        summary.get("exact_negative_capacity_exposure_from_refresh_trace")
    )
    ...
    return {
        "schema_version": "krk_candidate_generation_v4_next_runtime_boundary_review.v1",
        ...
        "summary": {
            ...
            "refresh_trace_row_count": summary.get("refresh_trace_row_count"),
        ...
        "boundary_assessment": {
            "current_candidate_generation_observation_sandbox_remains_valid": context_ready,
            ...
            "exact_move_provider_coverage_is_partial": exact_coverage_partial,
        ...
        "approved_now": {
            ...
            "keep_existing_default_off_observation_sandbox_available": context_ready,
        ...
        "interpretation": {
            "candidate_generation_context_is_ready_for_architecture_review": context_ready,
            "not_a_selector_packet": True,
            "reason": (
                "V4 integrates candidate-generation refresh traces and repair-monitor "
                "context as non-causal evidence. Policy-cell coverage is useful and "
                "negative exposure is clean, but exact move/provider coverage is partial "
                "and there are still no ownership selector labels."
            ),
        },
        "decision": {
            "status": (
                ...
                if context_ready
                else "candidate_generation_v4_next_runtime_boundary_needs_more_context"
            ),
            "selector_allowed": False,
            "guardrails_allowed": False,
            ...
            "recommended_next_step": (
                "non_causal_scope_gap_review_before_any_new_runtime_boundary"
                if context_ready
