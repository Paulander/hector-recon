#!/usr/bin/env python3
"""Build deduplicated state-local contrast labels including diverse v1 labels."""
    ...
    for path in FORCED_LABELS:
        ...
        for item in payload.get("labels") or []:
            if item.get("causal_status") != "non_causal_outcome_label":
                raise ValueError(f"{path}: label must remain non-causal")
            labels.append(item)
    return labels
...
def _sort_label(label: dict[str, Any]) -> tuple[int, int, str]:
    # Prefer converting evidence; otherwise keep deterministic shortest/oldest label.
    return (
        0 if label.get("result") == "mate" else 1,
        int(label.get("plies") or 999),
        str(label.get("job_id")),
    )


def _sort_proposal(row: dict[str, Any]) -> tuple[int, int, str]:
    return (
        0 if not row.get("stage7_challenge_row") else 1,
        int(row.get("global_raw_score_rank") or 999),
...
def build_dataset() -> dict[str, Any]:
    ...
    for label in _load_labels():
        labels_by_key[(str(label.get("state_id")), str(label.get("provider_id")))].append(label)

    proposals_by_key: dict[tuple[str, str], list[dict[str, Any]]] = defaultdict(list)
    for proposal in ranked.get("rows") or []:
        proposals_by_key[(str(proposal.get("state_id")), str(proposal.get("provider_id")))].append(proposal)

    rows = []
    matched_label_ids: set[str] = set()
    for key in sorted(set(labels_by_key) & set(proposals_by_key)):
        proposal = sorted(proposals_by_key[key], key=_sort_proposal)[0]
        label = sorted(labels_by_key[key], key=_sort_label)[0]
        matched_label_ids.add(str(label.get("job_id")))
        result_label = _result_label(label.get("result"))
        stage7_challenge = bool(proposal.get("stage7_challenge_row") or label.get("stage7_challenge_row"))
        rows.append({
            "schema_version": "krk_state_local_contrast_label.v2",
            "frame_id": proposal.get("frame_id"),
            "state_id": proposal.get("state_id"),
            "source_stage": proposal.get("source_stage") or label.get("source_stage"),
            "active_landmark_label": proposal.get("active_landmark_label"),
            "provider_id": proposal.get("provider_id"),
            "provider_family": proposal.get("provider_family") or label.get("provider_family"),
            "provider_maturity": proposal.get("provider_maturity") or label.get("provider_maturity"),
            "move_uci": proposal.get("move_uci"),
            "raw_score": proposal.get("raw_score"),
            "global_raw_score_rank": proposal.get("global_raw_score_rank") or label.get("global_raw_score_rank"),
            "provider_local_rank": proposal.get("provider_local_rank") or label.get("provider_local_rank"),
            "normalized_score": proposal.get("normalized_score") if proposal.get("normalized_score") is not None else label.get("normalized_score"),
            "frame_outcome": proposal.get("frame_outcome") or label.get("frame_outcome"),
            ...
            "usable_for_training": bool(result_label and not stage7_challenge),
    ...
    summary = {
        ...
        "training_state_count": len({row["state_id"] for row in rows if row["usable_for_training"]}),
        "contrast_label_counts": dict(Counter(str(row["contrast_label"]) for row in rows)),
        "training_contrast_label_counts": dict(
            Counter(str(row["contrast_label"]) for row in rows if row["usable_for_training"])
        ),
        "stage7_contrast_label_counts": dict(
            Counter(str(row["contrast_label"]) for row in rows if row["stage7_challenge_row"])
        ),
        ...
        "unmatched_forced_label_job_ids": sorted(unmatched_labels),
        "deduplicated_state_provider_rows": True,
        "label_key_count": len(label_keys),
        "proposal_key_count": len(proposal_keys),
    }
    dataset = {
        "schema_version": "krk_state_local_contrast_labels.v2",
        ...
        "decision": {
            ...
            "recommended_next_step": "probe_state_local_contrast_selector_v2",
