#!/usr/bin/env python3
"""Review protected missing-provider capacity execution manifest before labels."""
...
OUT_JSON = Path("reports/krk_protected_missing_provider_capacity_execution_manifest_review_v0.json")
OUT_MD = Path("reports/krk_protected_missing_provider_capacity_execution_manifest_review_v0.md")
...
def build_review() -> dict[str, Any]:
    ...
    if len(jobs) > 36:
        violations.append({"job_id": None, "violation": "job_count_exceeds_budget"})
    ...
    return {
        "schema_version": "krk_protected_missing_provider_capacity_execution_manifest_review.v0",
        ...
        "decision": {
            "status": (
                ...
                if labels_allowed
                else "protected_missing_provider_capacity_manifest_review_failed"
            ),
            "recommended_next_step": (
                "run_bounded_protected_missing_provider_capacity_labels"
                if labels_allowed
                else "fix_manifest_before_labels"
            ),
            "labels_allowed": labels_allowed,
            "runtime_work_allowed": False,
        },
...
def render_markdown(payload: dict[str, Any]) -> str:
    lines = [
        "# KRK Protected Missing-Provider Capacity Execution Manifest Review v0",
        "",
        f"Status: `{payload['decision']['status']}`",
        "",
        "Non-causal review of the protected missing-provider/capacity execution manifest.",
        "",
        "## Summary",
        "",
    ]
