#!/usr/bin/env python3
"""Run M14 context-specialized candidate generation plus local arbitration."""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--output",
        type=Path,
        default=Path("reports/autogrowth/krk_autogrowth_m14_context_specialized_candidates.json"),
    )
    parser.add_argument(
        "--smoke",
        action="store_true",
...
def main() -> int:
    args = parse_args()
    output = (
        Path("reports/autogrowth/krk_autogrowth_m14_context_specialized_candidates_smoke.json")
        if args.smoke and args.output == Path("reports/autogrowth/krk_autogrowth_m14_context_specialized_candidates.json")
        else args.output
    )
    ...
    print(
        ...
        f"context_features={payload['generation_summary']['context_feature_count']} "
