#!/usr/bin/env python3
"""Run TG25 foundation KRK curriculum checkpoint."""
...
from recon_lite_chess.autogrowth import FoundationCurriculumConfig, run_foundation_curriculum


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description=__doc__)
    parser.add_argument("--seed", type=int, default=20260612)
    parser.add_argument("--mate1-train-count", type=int, default=1000)
    parser.add_argument("--mate1-heldout-count", type=int, default=300)
    parser.add_argument("--mate1-mirror-count", type=int, default=120)
    ...
    parser.add_argument(
        "--output",
        type=Path,
        default=Path("reports/autogrowth/krk_autogrowth_tg25_foundation_curriculum.json"),
    )
...
def main() -> int:
    args = parse_args()
    default_output = Path("reports/autogrowth/krk_autogrowth_tg25_foundation_curriculum.json")
    output = (
        Path("reports/autogrowth/krk_autogrowth_tg25_foundation_curriculum_smoke.json")
    ...
    print(
        ...
        f"m3_updates={payload['mate1']['m3_update_count']} "
        f"m4={payload['mate1']['m4_consolidation_event_count']}"
    )
    if mate2.get("enabled"):
        ...
        print(f"mate2: skipped reason={mate2['reason']}")
    print(
        "decision: "
