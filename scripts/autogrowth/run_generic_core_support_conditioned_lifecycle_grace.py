#!/usr/bin/env python3
"""Run the frozen support-conditioned lifecycle-grace experiment with frozen admission."""
...
DEFAULT_OUTPUT = Path(
    ...
    "support_conditioned_lifecycle_grace_20260715.json"
)
CANDIDATE_SEEDS = tuple(range(20262401, 20262441))
ADMITTED_COUNT = 20
ADMISSION_OUTPUT = Path(
    "reports/autogrowth/generic_core/"
    "support_conditioned_lifecycle_grace_admission_20260715.json"
)
DEMANDS = (2,)
ARMS = (
    "fixed_8_ranked",
    "rent_batch_4_ranked",
    "rent_batch_4_graph_exact_two_review",
    "rent_batch_4_graph_exact_fixed_six",
    "rent_batch_4_graph_exact_conditioned_six",
)
RENT_ARMS = ARMS[1:]
REQUEST_ARMS = ARMS[2:]
...
def _configure_arm(
    ...
    if arm in RENT_ARMS:
        policy.enable_causal_rent(CausalRentConfig(
            ...
            lifecycle_grace_mode={
                "rent_batch_4_ranked": "two_review",
                "rent_batch_4_graph_exact_two_review": "two_review",
                "rent_batch_4_graph_exact_fixed_six": "fixed_six",
                "rent_batch_4_graph_exact_conditioned_six": (
...
def _target_candidate_diagnostics(
    ...
    """Describe the four exact cue-by-regime-1 candidates post hoc."""
    target_members = {
    ...
    unsupported_events = {
        ...
        "conditioned_grace_budget_exhausted",
        "conditioned_grace_no_progress",
        "conditioned_grace_request_inactive",
    }
    for action_id in task["door_action_ids"]:
        ...
        for members in sorted(target_members):
            ...
            if not matches:
                rows.append({
                    ...
                    "max_interval_support_high_water": 0,
                    "mature": False,
                    "ever_matured": False,
            ...
            review_support = [
                ...
                if event.get("support") is not None
            ]
            interval_high_waters = [
                int(event["interval_support_high_water"])
                for event in review_events
                if event.get("interval_support_high_water") is not None
            ]
            grace_audits = [
                event["grace_audit"] for event in review_events
                if event.get("grace_audit") is not None
            ]
            pruning_reason = next((
                str(event["pruning_reason"])
                for event in candidate_events
                if event.get("pruning_reason") is not None
            ), None)
            ...
            rows.append({
                ...
                "rent_evidence_support_high_water": (
                ...
                "birth_observation": candidate.born_observation,
                "birth_terminal_count": candidate.rent_birth_terminal_count,
                "birth_global_review": candidate.rent_birth_review_count,
                "birth_support": candidate.rent_birth_support,
                "trial_review_count": candidate.rent_review_count,
                ...
                "grace_request_emission_count": sum(
                    bool(audit.get("emitted")) for audit in grace_audits
                ),
                "grace_review_audits": grace_audits,
                "proposal_terminal_count": next((
                    event["terminal_count"] for event in candidate_events
                    if event.get("event") == "proposed"
                ), candidate.rent_birth_terminal_count),
                "max_review_support": max(review_support, default=0),
                "max_interval_support_high_water": max(
                    interval_high_waters, default=0
                ),
                "unsupported_death": any(
                    event.get("event") in unsupported_events
                    for event in candidate_events
                ),
                "pruning_reason": pruning_reason,
                "right_censored": candidate.rent_right_censored,
                "last_rent": candidate.last_rent,
                "last_margin_utility": candidate.last_margin_utility,
    ...
    return [
        {
            ...
            "rent_interval_support_high_water": (
                candidate.rent_interval_support_high_water
            ),
            "rent_review_count": candidate.rent_review_count,
            "rent_birth_terminal_count": candidate.rent_birth_terminal_count,
            "rent_birth_review_count": candidate.rent_birth_review_count,
            "rent_birth_support": candidate.rent_birth_support,
            "grace_extension_count": candidate.grace_extension_count,
            "rent_right_censored": candidate.rent_right_censored,
            "confirmation_count": candidate.confirmation_count,
            "exploration_request_count": candidate.exploration_request_count,
            "exploration_probe_benefit_count": (
                candidate.exploration_probe_benefit_count
...
def _request_topology_audit(
    ...
    grace_signatures = {}
    grace_enabled = (
        policy.causal_rent_config is not None
        and policy.causal_rent_config.lifecycle_grace_mode != "two_review"
    )
    for action_id, channel in policy.channels.items():
        trial_indices = {
            index
        ...
        grace_indices = set(channel.grace_terminal_node_ids)
        defer_indices = set(channel.defer_pruning_request_node_ids)
        expected_grace_indices = trial_indices if grace_enabled else set()
        if (
            grace_indices != expected_grace_indices
            or defer_indices != expected_grace_indices
        ):
            failures.append({
                "action_id": action_id,
                "failure": "grace_identity_mismatch",
                "expected_indices": sorted(expected_grace_indices),
                "terminal_indices": sorted(grace_indices),
                "request_indices": sorted(defer_indices),
            })
        for index in sorted(expected_grace_indices & grace_indices & defer_indices):
            terminal_ids = channel.grace_terminal_node_ids[index]
            request_id = channel.defer_pruning_request_node_ids[index]
            request = channel.graph.nodes.get(request_id)
            expected_children = set(terminal_ids.values())
            actual_children = {
                child_id
                for child_id, _ in channel.graph.get_sub_children(request_id)
            }
            terminal_names = {
                channel.graph.nodes[terminal_id].meta.get(
                    "internal_terminal"
                )
                for terminal_id in expected_children
                if terminal_id in channel.graph.nodes
            }
            if any((
                set(terminal_ids) != {
                    "evidence_deficit", "evidence_progress",
                    "request_active", "grace_budget_remaining",
                },
                request is None,
                request is not None and request.ntype.name != "SCRIPT",
                actual_children != expected_children,
                ...
                failures.append({
                    "action_id": action_id,
                    "candidate_index": index,
                    "failure": "grace_topology_firewall",
                    "request_id": request_id,
                    "expected_children": sorted(expected_children),
                    "actual_children": sorted(actual_children),
                    "terminal_names": sorted(map(str, terminal_names)),
                })
        evidence_signatures[action_id] = (
            channel.evidence_request_topology_signature()
        )
        grace_signatures[action_id] = (
            channel.lifecycle_grace_topology_signature()
        )
    return {
        "pass": not failures,
        "failure_count": len(failures),
        "failures": failures,
        "signatures": evidence_signatures,
        "grace_signatures": grace_signatures,
...
def _arm_result(
    ...
    *,
    ...
    if (
        policy.causal_rent_config is not None
        and not policy.causal_rent_phase_finalized
    ):
...
def _cell_invariants(cell: dict[str, object]) -> dict[str, object]:
    ...
    expected_modes = {
        "rent_batch_4_ranked": "ordinary_random",
        "rent_batch_4_graph_exact_two_review": "exact_support_directed",
        "rent_batch_4_graph_exact_fixed_six": "exact_support_directed",
        "rent_batch_4_graph_exact_conditioned_six": "exact_support_directed",
    }
    expected_lifecycles = {
        "rent_batch_4_ranked": "two_review",
        "rent_batch_4_graph_exact_two_review": "two_review",
    ...
    for arm in RENT_ARMS:
        ...
        lifecycles[arm] = config["lifecycle_grace_mode"]
    ...
    causal_bounds_ok = causal_bounds_ok and all((
        ...
        all(
            arms[arm]["causal_rent"]["phase_finalized"]
            for arm in RENT_ARMS
        ),
    ))
    ...
    expected_source = {
        arm: "exact_reservoir" for arm in REQUEST_ARMS
    }
    for arm in REQUEST_ARMS:
    ...
    for arm, expected_mode in (
        ...
        expected_backends = {
            "evidence_deficit": "candidate_local",
            "evidence_progress": (
                "fixed_active" if expected_mode == "fixed_six"
            ...
            "grace_budget_remaining": "candidate_local",
        }
        review_audits = [
            event["grace_audit"]
            for event in arms[arm]["causal_rent"]["events"]
            if event.get("event") == "review"
            and event.get("candidate_state") == "trial"
        ]
        grace_audit_ok = grace_audit_ok and all(
            audit.get("mode") == expected_mode
            and audit.get("terminal_backends") == expected_backends
            and set(audit.get("terminal_measurements", {})) == {
                "evidence_deficit", "evidence_progress",
                "request_active", "grace_budget_remaining",
            }
            and audit.get("request_node_id") is not None
            for audit in review_audits
        )
    right_censor_ok = True
    for arm in RENT_ARMS:
        snapshot = arms[arm]["causal_rent"]
        candidates = [
            candidate
        ...
        flagged = sum(
            bool(candidate.get("rent_right_censored"))
            for candidate in candidates
        )
        events = [
            event for event in snapshot["events"]
            if event.get("event") == "right_censored"
        ]
        right_censor_ok = right_censor_ok and all((
            snapshot["right_censored_count"] == flagged,
            len(events) == flagged,
            all(event.get("pruning_reason") is None for event in events),
        ))
    result = {
        ...
        "grace_audit_ok": grace_audit_ok,
        "right_censor_classification_ok": right_censor_ok,
...
def _gates(
    ...
    fixed_reference = summaries["2"]["fixed_8_ranked"]
    two_summary = summaries["2"][
        "rent_batch_4_graph_exact_two_review"
    ]
    fixed_six_summary = summaries["2"][
        "rent_batch_4_graph_exact_fixed_six"
    ]
    conditioned_summary = summaries["2"][
        "rent_batch_4_graph_exact_conditioned_six"
    ]
    ...
    def minimum_support(cell: dict[str, object], arm: str) -> int:
        return min(
            int(row["max_review_support"])
            for row in target_rows(cell, arm)
        )

    def all_four_supported(cell: dict[str, object], arm: str) -> bool:
        return all(
            int(row["max_review_support"]) >= 32
            for row in target_rows(cell, arm)
        )

    def all_four_mature_positive(cell: dict[str, object], arm: str) -> bool:
        return all(
            row["ever_matured"] and row["ever_positive_rent"]
            for row in target_rows(cell, arm)
        )

    two_arm = "rent_batch_4_graph_exact_two_review"
    fixed_six_arm = "rent_batch_4_graph_exact_fixed_six"
    conditioned_arm = "rent_batch_4_graph_exact_conditioned_six"
    two_min_support = [minimum_support(cell, two_arm) for cell in cells]
    fixed_six_min_support = [
        minimum_support(cell, fixed_six_arm) for cell in cells
    ]
    conditioned_min_support = [
        minimum_support(cell, conditioned_arm) for cell in cells
    ]
    conditioned_over_two = [
        conditioned - two
        for conditioned, two in zip(
            conditioned_min_support, two_min_support
        )
    ]
    conditioned_over_fixed_six = [
        conditioned - fixed_six
        for conditioned, fixed_six in zip(
            conditioned_min_support, fixed_six_min_support
        )
    ]

    two_supported_count = sum(
        all_four_supported(cell, two_arm) for cell in cells
    )
    fixed_six_supported_count = sum(
        all_four_supported(cell, fixed_six_arm) for cell in cells
    )
    conditioned_supported_count = sum(
        all_four_supported(cell, conditioned_arm) for cell in cells
    )
    two_mature_count = sum(
        all_four_mature_positive(cell, two_arm) for cell in cells
    )
    fixed_six_mature_count = sum(
        all_four_mature_positive(cell, fixed_six_arm) for cell in cells
    )
    conditioned_mature_count = sum(
        all_four_mature_positive(cell, conditioned_arm) for cell in cells
    )
    conditioned_unsupported_deaths = sum(
        bool(row["unsupported_death"])
        for cell in cells
        for row in target_rows(cell, conditioned_arm)
    )
    conditioned_right_censored = sum(
        bool(row["right_censored"])
        for cell in cells
        for row in target_rows(cell, conditioned_arm)
    )

    conditioned_vs_fixed_reference_behavior = [
        _minimum_score(cell, conditioned_arm)
        - _minimum_score(cell, "fixed_8_ranked")
        for cell in cells
    ]
    conditioned_vs_fixed_six_behavior = [
        _minimum_score(cell, conditioned_arm)
        - _minimum_score(cell, fixed_six_arm)
        for cell in cells
    ]
    fixed_six_occupancy = [
        float(cell["arms"][fixed_six_arm]["causal_rent"][
            "mean_live_trial_occupancy"
        ])
        for cell in cells
    ]
    conditioned_occupancy = [
        float(cell["arms"][conditioned_arm]["causal_rent"][
            "mean_live_trial_occupancy"
        ])
        for cell in cells
    ]
    median_fixed_six_occupancy = statistics.median(fixed_six_occupancy)
    median_conditioned_occupancy = statistics.median(conditioned_occupancy)
    occupancy_reduction_fraction = (
        1.0 - median_conditioned_occupancy / median_fixed_six_occupancy
        if median_fixed_six_occupancy > 0.0
        else 0.0
    )
    challenger_blocks_lower_count = sum(
        cell["arms"][conditioned_arm]["causal_rent"][
            "challenger_block_count"
        ]
        < cell["arms"][fixed_six_arm]["causal_rent"][
            "challenger_block_count"
        ]
        for cell in cells
    )
    displaced_proposals_lower_count = sum(
        cell["arms"][conditioned_arm]["causal_rent"][
    ...
    more_life_effect = all((
        sum(value > 0 for value in conditioned_over_two) >= 14,
        statistics.median(conditioned_over_two) >= 12,
    ))
    conditioned_evidence = all((
        conditioned_supported_count >= 16,
        conditioned_unsupported_deaths <= 4,
    ))
    conditioned_maturation = conditioned_mature_count >= 16
    self_regulation_vs_fixed_six = all((
        conditioned_supported_count >= fixed_six_supported_count - 2,
        conditioned_mature_count >= fixed_six_mature_count - 2,
        statistics.median(conditioned_over_fixed_six) >= -2,
        statistics.median(conditioned_vs_fixed_six_behavior) >= -0.05,
        occupancy_reduction_fraction >= 0.10,
        challenger_blocks_lower_count >= 14,
        displaced_proposals_lower_count >= 14,
    ))
    behavior = all((
        conditioned_summary["median_old_joint_success"] >= 0.85,
        conditioned_summary["median_new_joint_success"] >= 0.85,
        conditioned_summary["both_at_least_0_85_task_count"] >= 16,
        statistics.median(conditioned_vs_fixed_reference_behavior) >= -0.05,
    ))
    stability = all((
        fixed_reference["median_old_joint_success"] >= 0.85,
        fixed_reference["median_new_joint_success"] >= 0.85,
        fixed_reference["both_at_least_0_85_task_count"] >= 16,
    ))

    exact_explorations = [
        cell["arms"][arm]["exploration"]
        for cell in cells for arm in REQUEST_ARMS
    ]
    priority_total = sum(
        exploration["zero_request_opportunity_count"]
        + exploration["one_request_opportunity_count"]
    ...
    support_conditioned_self_regulation = all((
        measurement_integrity,
        more_life_effect,
        conditioned_evidence,
        conditioned_maturation,
        self_regulation_vs_fixed_six,
    ))
    behavioral_readiness = all((
        support_conditioned_self_regulation,
        behavior,
        stability,
    ))
    gate_rows = {
        "measurement_integrity": measurement_integrity,
        "more_life_effect": more_life_effect,
        "conditioned_evidence": conditioned_evidence,
        "conditioned_maturation": conditioned_maturation,
        "self_regulation_vs_fixed_six": self_regulation_vs_fixed_six,
        "behavior": behavior,
        "stability": stability,
        "priority_exposure_identified_descriptive": priority_identified,
    }
    return {
        "gate_rows": gate_rows,
        "support_conditioned_self_regulation": (
        ...
        "development_support": support_conditioned_self_regulation,
        "native_transfer_authorized": False,
        "priority_status": (
            "identified" if priority_identified else "not_identified"
        ),
        "two_review_all_four_supported_task_count": two_supported_count,
        "fixed_six_all_four_supported_task_count": (
            fixed_six_supported_count
        ),
        "conditioned_all_four_supported_task_count": (
            conditioned_supported_count
        ),
        "two_review_all_four_mature_positive_task_count": two_mature_count,
        "fixed_six_all_four_mature_positive_task_count": fixed_six_mature_count,
        "conditioned_all_four_mature_positive_task_count": (
            conditioned_mature_count
        ),
        "conditioned_unsupported_lifecycle_death_count": (
            conditioned_unsupported_deaths
        ),
        "conditioned_right_censored_target_count": (
            conditioned_right_censored
        ),
        "conditioned_over_two_support_task_count": sum(
            value > 0 for value in conditioned_over_two
        ),
        "median_conditioned_min_support_advantage_over_two": (
            statistics.median(conditioned_over_two)
        ),
        "median_conditioned_min_support_difference_vs_fixed_six": (
            statistics.median(conditioned_over_fixed_six)
        ),
        "median_conditioned_behavior_difference_vs_fixed_six": (
            statistics.median(conditioned_vs_fixed_six_behavior)
        ),
        "median_conditioned_behavior_difference_vs_fixed_reference": (
            statistics.median(conditioned_vs_fixed_reference_behavior)
        ),
        "median_fixed_six_live_trial_occupancy": (
            median_fixed_six_occupancy
        ),
        "median_conditioned_live_trial_occupancy": (
            median_conditioned_occupancy
        ),
        "conditioned_occupancy_reduction_fraction": (
            occupancy_reduction_fraction
        ),
        "conditioned_lower_challenger_blocks_task_count": (
            challenger_blocks_lower_count
        ),
        "conditioned_lower_displaced_proposals_task_count": (
            displaced_proposals_lower_count
        ),
        "priority_exposure": {
        ...
        "two_review_summary": two_summary,
        "fixed_six_summary": fixed_six_summary,
...
def main() -> int:
    ...
    admission_payload = {
        "schema_version": (
            "recon_support_conditioned_lifecycle_grace_admission_manifest.v1"
        ),
    ...
    if not admission_pass:
        payload = {
            "schema_version": (
                "recon_support_conditioned_lifecycle_grace_raw.v1"
            ),
