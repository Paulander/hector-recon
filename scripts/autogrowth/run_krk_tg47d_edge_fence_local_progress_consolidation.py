#!/usr/bin/env python3
"""Run TG47d local progress-terminal edge/fence consolidation diagnostic."""
...
DEFAULT_OUTPUT_DIR = "reports/autogrowth/clean_slate_krk/tg47d_edge_fence_local_progress_consolidation"
...
def main() -> None:
    ...
    if not args.fresh_extension:
        raise SystemExit("TG47d requires --fresh-extension")

    output_dir = args.output_dir
    config = CleanEdgeFenceStageConfig(
        checkpoint_name="TG47d_edge_fence_local_progress_consolidation",
        schema_version="krk_tg47d_edge_fence_local_progress_consolidation.v0",
        progress_schema_version="krk_tg47d_edge_fence_local_progress_consolidation_progress.v0",
        run_scale_label=args.run_scale_label or _run_scale_label(args),
        output_dir=output_dir,
        output_path=f"{output_dir}/krk_tg47d_edge_fence_local_progress_consolidation.json",
        progress_path=f"{output_dir}/krk_tg47d_edge_fence_local_progress_consolidation_progress.json",
        markdown_path=f"{output_dir}/krk_tg47d_edge_fence_local_progress_consolidation.md",
        train_trace_path=f"{output_dir}/pools/tg47d_train_traces.jsonl.gz",
        eval_trace_path=f"{output_dir}/pools/tg47d_eval_traces.jsonl.gz",
        failure_pool_path=f"{output_dir}/pools/tg47d_failure_pool.jsonl.gz",
        online_trace_path=f"{output_dir}/pools/tg47d_online_episodes.jsonl.gz",
        m4_audit_log_path=f"{output_dir}/pools/tg47d_m4_audit.jsonl.gz",
        graph_summary_path=f"{output_dir}/pools/tg47d_graph_summary.json",
        promoted_edge_fence_artifact_path=f"{output_dir}/promoted_tg47d_edge_fence.json",
        seed=args.seed,
