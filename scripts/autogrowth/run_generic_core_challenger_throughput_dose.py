#!/usr/bin/env python3
"""Run the frozen generic-core challenger-throughput dose experiment."""
...
DEFAULT_OUTPUT = Path(
    ...
    "challenger_throughput_dose_20260713.json"
)
SEEDS = tuple(range(20262001, 20262021))
DEMANDS = (0, 1, 2)
ARMS = (
    "fixed_8_ranked",
    "rent_batch_1_ranked",
...
CONTRACT = Path(
    "docs/autogrowth/"
    "GENERIC_CORE_CHALLENGER_THROUGHPUT_DOSE_WORK_PACKAGE_20260713.md"
)
...
def _configure_arm(
    ...
    if arm in RENT_ARMS:
        temporary_challenger_allowance = {
            "rent_batch_1_ranked": 1,
            "rent_batch_2_ranked": 2,
            "rent_batch_4_ranked": 4,
            "rent_batch_4_shuffled": 4,
        }[arm]
        policy.enable_causal_rent(CausalRentConfig(
            temporary_challenger_allowance=temporary_challenger_allowance,
            proposal_mode=(
                "rank_shuffled"
                if arm == "rent_batch_4_shuffled"
                else "residual_ranked"
            ),
        ))
...
def _replay_reference_stream(
    ...
    for episode in stream:
        terminal_return = float(episode["terminal_return"])
        policy.terminal_return_sum += terminal_return
        policy.terminal_count += 1
        decisions = episode["decisions"]
        policy.terminal_trace_lengths.append(len(decisions))
        for decision in decisions:
            target = terminal_return * (
                policy.config.discount ** int(decision["elapsed_steps"])
            )
            record = LifetimeDecisionRecord(
                sequence=policy.experience_reservoir.seen_count,
                action_id=decision["action_id"],
                active_atom_ids=tuple(decision["active_atom_ids"]),
                legal_action_ids=tuple(decision["legal_action_ids"]),
                decision_scores=tuple(
                    tuple(item) for item in decision["decision_scores"]
                ),
                target=target,
                discount=policy.config.discount,
                elapsed_steps=int(decision["elapsed_steps"]),
            )
            policy.experience_reservoir.add(record)
            policy.channels[record.action_id].observe(
                record.active_atom_ids, target
            )
            policy.credited_decision_count += 1
            digest.update(
                json.dumps(asdict(record), sort_keys=True).encode()
            )
        policy._causal_rent_tick()
    return digest.hexdigest()


def _shadow_result(policy: EpisodicCompositionPolicy) -> dict[str, object]:
    candidate_stats = []
    for action_id, channel in policy.channels.items():
        for index, candidate in enumerate(channel.learner.candidates):
            stats = policy.candidate_rent_stats(action_id, index)
    ...
    return {
        "mean_positive_counterfactual_rent": (
            sum(positive) / len(positive) if positive else 0.0
        ),
        "adequately_supported_candidate_count": len(positive),
        "candidate_stats": candidate_stats,
        "causal_rent": policy.snapshot()["causal_rent"],
    }
...
def _cell_invariants(cell: dict[str, object]) -> dict[str, object]:
    ...
    for arm in RENT_ARMS:
        ...
        batch_size = config["temporary_challenger_allowance"]
        configured_batch_sizes[arm] = batch_size
        causal_bounds_ok = causal_bounds_ok and all((
            config["global_capacity"] == 32,
            snapshot["maximum_global_live_candidate_count"]
            <= 32 + batch_size,
            snapshot["global_mature_count"] <= 32,
            snapshot["global_live_count"] - snapshot["global_mature_count"]
            <= batch_size,
        ))
    causal_bounds_ok = causal_bounds_ok and configured_batch_sizes == {
        "rent_batch_1_ranked": 1,
        "rent_batch_2_ranked": 2,
        "rent_batch_4_ranked": 4,
        "rent_batch_4_shuffled": 4,
    }
    forbidden = {"role", "demand", "correctness", "correct_action", "cohort"}
...
def _gates(
    ...
    cells_by_demand = {
        demand: [row["demands"][str(demand)] for row in task_rows]
        for demand in DEMANDS
    }
    m2 = cells_by_demand[2]
    batch_4_over_batch_1 = [
        _minimum_score(cell, "rent_batch_4_ranked")
        - _minimum_score(cell, "rent_batch_1_ranked")
        for cell in m2
    ]
    batch_4_over_batch_2 = [
        _minimum_score(cell, "rent_batch_4_ranked")
        - _minimum_score(cell, "rent_batch_2_ranked")
        for cell in m2
    ]
    batch_4_vs_fixed = [
        _minimum_score(cell, "rent_batch_4_ranked")
    ...
    ranked_over_shuffled = [
        _minimum_score(cell, "rent_batch_4_ranked")
        - _minimum_score(cell, "rent_batch_4_shuffled")
        for cell in m2
    ]
    m2_occupancy = {
        arm: [
    ...
    strict_topology_dose_count = sum(
        low < middle < high
        for low, middle, high in zip(
            m2_occupancy["rent_batch_1_ranked"],
            m2_occupancy["rent_batch_2_ranked"],
            m2_occupancy["rent_batch_4_ranked"],
        )
    )
    batch_4_minus_batch_1_occupancy = [
        high - low
        for low, high in zip(
            m2_occupancy["rent_batch_1_ranked"],
            m2_occupancy["rent_batch_4_ranked"],
    ...
    demand_occupancy = {
        demand: [
            cell["arms"]["rent_batch_4_ranked"]
            ["global_mature_candidate_count"]
            for cell in cells_by_demand[demand]
        ]
        for demand in DEMANDS
    }
    demand_monotonic_count = sum(
        low <= middle <= high
        for low, middle, high in zip(
            demand_occupancy[0],
            demand_occupancy[1],
    ...
    demand_m2_minus_m0 = [
        high - low
        for low, high in zip(demand_occupancy[0], demand_occupancy[2])
    ]
    m0_fixed_minus_batch_4 = [
        cell["arms"]["fixed_8_ranked"]["global_mature_candidate_count"]
        - cell["arms"]["rent_batch_4_ranked"]
        ["global_mature_candidate_count"]
        for cell in cells_by_demand[0]
    ]
    shadow_advantage = [
        cell["shadows"]["ranked"]["mean_positive_counterfactual_rent"]
        - cell["shadows"]["shuffled"]
        ["mean_positive_counterfactual_rent"]
        for cell in m2
    ]

    fixed = summaries["2"]["fixed_8_ranked"]
    batch_1 = summaries["2"]["rent_batch_1_ranked"]
    batch_2 = summaries["2"]["rent_batch_2_ranked"]
    batch_4 = summaries["2"]["rent_batch_4_ranked"]
    batch_4_m0 = summaries["0"]["rent_batch_4_ranked"]
    topology_medians = [
        batch_1["median_mature_occupancy"],
        batch_2["median_mature_occupancy"],
        batch_4["median_mature_occupancy"],
    ]

    safety_and_identity = all(
        cell["arms"][arm]["causal_rent"]
        ["safety_ceiling_bind_count"] == 0
        for demand in DEMANDS
        for cell in cells_by_demand[demand]
        for arm in RENT_ARMS
    ) and all(
    ...
    gate_rows = {
        "fixed_8_ceiling_replication": all((
            fixed["median_old_joint_success"] >= 0.85,
            fixed["median_new_joint_success"] >= 0.85,
            fixed["both_at_least_0_85_task_count"] >= 16,
        )),
        "batch_1_manipulation_check": all((
            batch_1["median_mature_occupancy"] <= 12,
            (
                batch_1["median_old_joint_success"] < 0.85
                or batch_1["median_new_joint_success"] < 0.85
                or batch_1["both_at_least_0_85_task_count"] < 16
            ),
        )),
        "topology_dose": all((
            topology_medians[0] < topology_medians[1]
            < topology_medians[2],
            statistics.median(batch_4_minus_batch_1_occupancy) >= 6,
            strict_topology_dose_count >= 14,
        )),
        "performance_dose": all((
            sum(value > 0.0 for value in batch_4_over_batch_1) >= 14,
            statistics.median(batch_4_over_batch_1) >= 0.15,
            sum(value > 0.0 for value in batch_4_over_batch_2) >= 12,
            statistics.median(batch_4_over_batch_2) > 0.0,
        )),
        "fixed_capacity_noninferiority": all((
            batch_4["median_old_joint_success"] >= 0.85,
            batch_4["median_new_joint_success"] >= 0.85,
            batch_4["both_at_least_0_85_task_count"] >= 16,
            statistics.median(batch_4_vs_fixed) >= -0.05,
        )),
        "nomination_selectivity": all((
            sum(value > 0.0 for value in ranked_over_shuffled) >= 14,
            statistics.median(ranked_over_shuffled) >= 0.10,
            sum(value > 0.0 for value in shadow_advantage) >= 14,
            statistics.median(shadow_advantage) > 0.0,
        )),
        "demand_allocation": all((
            statistics.median(demand_occupancy[0])
            <= statistics.median(demand_occupancy[1])
            <= statistics.median(demand_occupancy[2]),
            demand_monotonic_count >= 14,
            statistics.median(demand_m2_minus_m0) >= 6,
        )),
        "low_demand_compression": all((
            statistics.median(m0_fixed_minus_batch_4) >= 4,
            batch_4_m0["median_old_joint_success"] >= 0.85,
            batch_4_m0["median_new_joint_success"] >= 0.85,
        )),
    ...
    return {
        ...
        "m2_mature_occupancy_medians_by_batch": topology_medians,
        "strict_topology_dose_seed_count": strict_topology_dose_count,
        "median_batch_4_minus_batch_1_occupancy": statistics.median(
            batch_4_minus_batch_1_occupancy
        ),
        "batch_4_over_batch_1_positive_task_count": sum(
            value > 0.0 for value in batch_4_over_batch_1
        ),
        "median_batch_4_advantage_over_batch_1": statistics.median(
            batch_4_over_batch_1
        ),
        "batch_4_over_batch_2_positive_task_count": sum(
            value > 0.0 for value in batch_4_over_batch_2
        ),
        "median_batch_4_advantage_over_batch_2": statistics.median(
            batch_4_over_batch_2
        ),
        "median_batch_4_difference_vs_fixed": statistics.median(
            batch_4_vs_fixed
        ),
        "ranked_over_shuffled_positive_task_count": sum(
            value > 0.0 for value in ranked_over_shuffled
        ),
        "median_ranked_advantage_over_shuffled": statistics.median(
            ranked_over_shuffled
        ),
        "demand_monotonic_seed_count": demand_monotonic_count,
        "median_demand_m2_minus_m0_occupancy": statistics.median(
            demand_m2_minus_m0
        ),
        "median_m0_fixed_minus_batch_4_occupancy": statistics.median(
            m0_fixed_minus_batch_4
        ),
        "shadow_ranked_advantage_task_count": sum(
            value > 0.0 for value in shadow_advantage
        ),
        "median_shadow_ranked_advantage": statistics.median(
            shadow_advantage
        ),
    }
...
def main() -> int:
    ...
    if not phase0_pass:
        payload = {
            "schema_version": "recon_challenger_throughput_dose_raw.v1",
    ...
    for task_index, (task, phase0_policy, checkpoint_hash) in enumerate(
        ...
        for demand in DEMANDS:
            ...
            for arm in ARMS:
                ...
                trajectory = []
                captured_stream = [] if arm == "fixed_8_ranked" else None
                ...
                for episode, experience_row in enumerate(
                    ...
                    key_action, door_action, door_atoms, captured = _train_episode(
                        policy,
                        task,
                        experience_row,
                        demand,
                        capture_trace=captured_stream is not None,
                    )
                    ...
                    if captured_stream is not None:
                        ...
                        captured_stream.append(captured)
                ...
                if captured_stream is not None:
                    reference_stream = captured_stream
            ...
            assert reference_stream is not None
            shadows = {}
            shadow_digests = []
            for shadow_name, arm_name in (
                ("ranked", "rent_batch_4_ranked"),
                ("shuffled", "rent_batch_4_shuffled"),
            ):
                shadow = deepcopy(phase0_policy)
                _configure_arm(shadow, arm_name)
                digest = _replay_reference_stream(shadow, reference_stream)
                shadow_digests.append(digest)
                shadows[shadow_name] = _shadow_result(shadow)
                shadows[shadow_name]["reference_stream_sha256"] = digest
                _write_json(partial, {
                    "status": "shadow_checkpoint",
                    "completed_seed": task["seed"],
                    "completed_demand": demand,
                    "completed_shadow": shadow_name,
                    "completed_arm_rows": arm_rows,
                    "completed_shadows": shadows,
                    "task_rows": task_rows,
                })
            ...
            cell = {
                ...
                "arms": arm_rows,
                "shadows": shadows,
                "shadow_reference_stream_parity": len(set(shadow_digests)) == 1,
            }
            cell["invariants"] = _cell_invariants(cell)
            cell["invariants"]["shadow_reference_stream_parity"] = cell[
                "shadow_reference_stream_parity"
            ]
            cell["invariants"]["pass"] = all(
                value for key, value in cell["invariants"].items()
                if key != "pass"
            )
