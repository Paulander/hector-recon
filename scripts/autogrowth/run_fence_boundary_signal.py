#!/usr/bin/env python3
"""Run TG26g fence boundary local signal checkpoint."""
...
def main() -> int:
    args = parse_args()
    output = args.output
    if args.smoke and output == Path("reports/autogrowth/krk_autogrowth_tg26g_fence_boundary_signal.json"):
        output = Path("reports/autogrowth/krk_autogrowth_tg26g_fence_boundary_signal_smoke.json")
    result = run_fence_boundary_signal(
        config=FenceBoundarySignalConfig(
            ...
            tg26f_reference_artifact_path=str(args.tg26f_reference_artifact),
        )
    )
