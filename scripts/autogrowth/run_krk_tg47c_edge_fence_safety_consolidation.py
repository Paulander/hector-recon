#!/usr/bin/env python3
"""Run TG47c edge/fence safety-dominance and consolidation diagnostic."""
...
DEFAULT_OUTPUT_DIR = "reports/autogrowth/clean_slate_krk/tg47c_edge_fence_safety_consolidation"
...
def main() -> None:
    ...
    if not args.fresh_extension:
        raise SystemExit("TG47c requires --fresh-extension")

    output_dir = args.output_dir
    config = CleanEdgeFenceStageConfig(
        checkpoint_name="TG47c_edge_fence_safety_consolidation",
        schema_version="krk_tg47c_edge_fence_safety_consolidation.v0",
        progress_schema_version="krk_tg47c_edge_fence_safety_consolidation_progress.v0",
        run_scale_label=args.run_scale_label or _run_scale_label(args),
        output_dir=output_dir,
        output_path=f"{output_dir}/krk_tg47c_edge_fence_safety_consolidation.json",
        progress_path=f"{output_dir}/krk_tg47c_edge_fence_safety_consolidation_progress.json",
        markdown_path=f"{output_dir}/krk_tg47c_edge_fence_safety_consolidation.md",
        train_trace_path=f"{output_dir}/pools/tg47c_train_traces.jsonl.gz",
        eval_trace_path=f"{output_dir}/pools/tg47c_eval_traces.jsonl.gz",
        failure_pool_path=f"{output_dir}/pools/tg47c_failure_pool.jsonl.gz",
        online_trace_path=f"{output_dir}/pools/tg47c_online_episodes.jsonl.gz",
        m4_audit_log_path=f"{output_dir}/pools/tg47c_m4_audit.jsonl.gz",
        graph_summary_path=f"{output_dir}/pools/tg47c_graph_summary.json",
        promoted_edge_fence_artifact_path=f"{output_dir}/promoted_tg47c_edge_fence.json",
        seed=args.seed,
