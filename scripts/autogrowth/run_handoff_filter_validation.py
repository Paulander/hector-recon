#!/usr/bin/env python3
"""Run TG26d handoff-filter validation checkpoint."""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--train-pool-size", type=int, default=160)
    ...
    parser.add_argument(
        "--output",
        type=Path,
        default=Path("reports/autogrowth/krk_autogrowth_tg26d_handoff_filter_validation.json"),
    )
...
def main() -> int:
    args = parse_args()
    output = args.output
    if args.smoke and output == Path("reports/autogrowth/krk_autogrowth_tg26d_handoff_filter_validation.json"):
        output = Path("reports/autogrowth/krk_autogrowth_tg26d_handoff_filter_validation_smoke.json")
    result = run_handoff_filter_validation(
        config=HandoffFilterValidationConfig(
            ...
            train_pool_size=12 if args.smoke else args.train_pool_size,
            train_chunk_size=24 if args.smoke else args.train_chunk_size,
            eval_window_size=6 if args.smoke else args.eval_window_size,
            ...
            max_samples=4 if args.smoke else args.max_samples,
    ...
    print(
        "artifact_integrity: "
        f"exists={payload['artifact_integrity']['exists']} "
        f"parseable={payload['artifact_integrity']['parseable_full_json']} "
        f"bytes={payload['artifact_integrity'].get('byte_size')}"
    )
    for stage in payload["stages"]:
        ...
        for name, metrics in stage["eval_slices"].items():
            print(
                ...
                f"direct_mate={metrics['direct_mate_count']} "
                f"rook_loss={metrics['rook_loss_count']} "
                f"confinement_regression={metrics['confinement_regression_count']}"
            )
    ...
    print(
        ...
        f"filtered={decision['filtered_runway_signal']} "
        f"unfiltered={decision['unfiltered_generalization_signal']} "
        f"boundary={decision['boundary_bridge_signal']} "
