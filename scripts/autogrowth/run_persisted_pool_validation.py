#!/usr/bin/env python3
"""Run TG26e persisted edge/fence pool validation checkpoint."""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--train-chunk-size", type=int, default=96)
...
def main() -> int:
    args = parse_args()
    output = args.output
    if args.smoke and output == Path("reports/autogrowth/krk_autogrowth_tg26e_persisted_pool_validation.json"):
        output = Path("reports/autogrowth/krk_autogrowth_tg26e_persisted_pool_validation_smoke.json")
    ...
    for pool_id, pool in payload["pools"].items():
        stats = pool["stats"]
        print(
            f"{pool_id}: accepted={stats['accepted_count']} attempts={stats['generation_attempts']} "
    ...
    print(
        ...
        f"edge_survived={decision['edge_generalization_survived']} "
        f"fence_transfer={decision['fence_transferred_beyond_filtered']} "
