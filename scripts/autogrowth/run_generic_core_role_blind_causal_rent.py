#!/usr/bin/env python3
"""Run the frozen role-blind causal-rent demand-gradient experiment."""
...
DEFAULT_OUTPUT = Path(
    ...
    "role_blind_causal_rent_20260712.json"
)
SEEDS = tuple(range(20261901, 20261921))
DEMANDS = (0, 1, 2)
ARMS = (
    "fixed_8_ranked",
    "fixed_8_random",
    "causal_rent_ranked",
    "causal_rent_shuffled",
)
...
CONTRACT = Path(
    "docs/autogrowth/"
    "GENERIC_CORE_ROLE_BLIND_CAUSAL_RENT_WORK_PACKAGE_20260712.md"
)
...
def _configure_arm(
    ...
    random_control = arm == "fixed_8_random"
    for channel in policy.channels.values():
    ...
    if arm.startswith("causal_rent"):
        policy.enable_causal_rent(CausalRentConfig(
            proposal_mode=(
                "rank_shuffled"
                if arm == "causal_rent_shuffled"
...
def _cell_invariants(cell: dict[str, object]) -> dict[str, object]:
    ...
    fixed_capacity_ok = all(
        ...
        for arm in ("fixed_8_ranked", "fixed_8_random")
        for channel in arms[arm]["channels"].values()
    )
    causal_bounds_ok = all(
        arms[arm]["causal_rent"]["maximum_global_live_candidate_count"] <= 33
        and arms[arm]["causal_rent"]["global_mature_count"] <= 32
        for arm in ("causal_rent_ranked", "causal_rent_shuffled")
    )
...
def _gates(
    ...
    fixed_over_random = [
        ...
        - _minimum_score(cell, "fixed_8_random")
        for cell in m2
    ]
    rent_vs_fixed = [
        _minimum_score(cell, "causal_rent_ranked")
        - _minimum_score(cell, "fixed_8_ranked")
        for cell in m2
    ]
    rent_over_shuffled = [
        _minimum_score(cell, "causal_rent_ranked")
        - _minimum_score(cell, "causal_rent_shuffled")
        for cell in m2
    ]
    occupancy = {
        demand: [
            cell["arms"]["causal_rent_ranked"]
            ["global_mature_candidate_count"]
            for cell in cells_by_demand[demand]
        ]
        for demand in DEMANDS
    }
    monotonic_seed_count = sum(
        occupancy[0][index] <= occupancy[1][index] <= occupancy[2][index]
        for index in range(len(task_rows))
    )
    m2_minus_m0 = [
        high - low for low, high in zip(occupancy[0], occupancy[2])
    ]
    m0_fixed_minus_rent = [
    ...
    fixed_summary = summaries["2"]["fixed_8_ranked"]
    rent_summary = summaries["2"]["causal_rent_ranked"]
    rent_m0 = summaries["0"]["causal_rent_ranked"]
    gate_rows = {
        "fixed_8_replication_and_selectivity": all((
            fixed_summary["median_old_joint_success"] >= 0.85,
            fixed_summary["median_new_joint_success"] >= 0.85,
            fixed_summary["both_at_least_0_85_task_count"] >= 16,
            sum(value > 0.0 for value in fixed_over_random) >= 14,
            statistics.median(fixed_over_random) >= 0.10,
        )),
        "causal_rent_noninferior_to_fixed_8": all((
            rent_summary["median_old_joint_success"] >= 0.85,
            rent_summary["median_new_joint_success"] >= 0.85,
            rent_summary["both_at_least_0_85_task_count"] >= 16,
            statistics.median(rent_vs_fixed) >= -0.05,
        )),
        "causal_rent_selectivity": all((
            sum(value > 0.0 for value in rent_over_shuffled) >= 14,
            statistics.median(rent_over_shuffled) >= 0.10,
        )),
        "hidden_demand_occupancy_gradient": all((
            statistics.median(occupancy[0])
            <= statistics.median(occupancy[1])
            <= statistics.median(occupancy[2]),
            monotonic_seed_count >= 14,
            statistics.median(m2_minus_m0) >= 4,
        )),
        "m0_sheds_unneeded_topology": all((
            statistics.median(m0_fixed_minus_rent) >= 4,
            rent_m0["median_old_joint_success"] >= 0.85,
            rent_m0["median_new_joint_success"] >= 0.85,
        )),
        "common_experience_nomination_selectivity": all((
            sum(value > 0.0 for value in shadow_advantage) >= 14,
            statistics.median(shadow_advantage) > 0.0,
        )),
        "safety_ceiling_never_binds": all(
    ...
    return {
        ...
        "fixed_over_random_positive_task_count": sum(
            value > 0.0 for value in fixed_over_random
        ),
        "median_fixed_advantage_over_random": statistics.median(
            fixed_over_random
        ),
        "median_rent_difference_vs_fixed": statistics.median(rent_vs_fixed),
        "rent_over_shuffled_positive_task_count": sum(
            value > 0.0 for value in rent_over_shuffled
        ),
        "median_rent_advantage_over_shuffled": statistics.median(
            rent_over_shuffled
        ),
        "monotonic_occupancy_seed_count": monotonic_seed_count,
        "median_m2_minus_m0_occupancy": statistics.median(m2_minus_m0),
        "median_m0_fixed_minus_rent_occupancy": statistics.median(
            m0_fixed_minus_rent
        ),
...
def main() -> int:
    ...
    if not phase0_pass:
        payload = {
            "schema_version": "recon_role_blind_causal_rent_raw.v1",
    ...
    for task_index, (task, phase0_policy, checkpoint_hash) in enumerate(
        ...
        for demand in DEMANDS:
            ...
            for shadow_name, arm_name in (
                ("ranked", "causal_rent_ranked"),
                ("shuffled", "causal_rent_shuffled"),
            ):
