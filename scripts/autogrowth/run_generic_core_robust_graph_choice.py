#!/usr/bin/env python3
"""Run the once-frozen robust graph-choice development experiment."""
...
SEEDS = tuple(range(20260821, 20260841))
TRAIN_EPISODES = 2048
...
def _run_arm(
    *,
    ...
    estimates = {
        action_id: asdict(policy.memory.estimate(action_id))
    ...
    return {
        ...
        "final_greedy_action_id": policy.greedy_action(),
        ...
        "retained_returns": {
            action_id: list(policy.memory.states[action_id].returns)
            if action_id in policy.memory.states
            else []
            for action_id in policy.action_ids
        },
    }
...
def main() -> int:
    ...
    for seed in SEEDS:
        ...
        task_rows.append({
            ...
            "evaluation_responses_sha256": task["evaluation_responses_sha256"],
            "mean": mean_arm,
            "lower_tail": lower_tail,
        })

    payload = {
        "schema_version": "recon_generic_robust_graph_choice_raw.v1",
