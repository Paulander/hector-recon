#!/usr/bin/env python3
"""Run TG28d foundation-backed bridge frontier checkpoint."""
...
def main() -> int:
    ...
    parser.add_argument(
        "--output",
        type=Path,
        default=Path("reports/autogrowth/krk_autogrowth_tg28d_foundation_backed_bridge_frontier.json"),
    )
    parser.add_argument(
        "--progress-output",
        type=str,
        default="reports/autogrowth/krk_autogrowth_tg28d_foundation_backed_bridge_frontier_progress.json",
    )
    ...
    output = (
        Path("reports/autogrowth/krk_autogrowth_tg28d_foundation_backed_bridge_frontier_smoke.json")
        if args.smoke and args.output == Path("reports/autogrowth/krk_autogrowth_tg28d_foundation_backed_bridge_frontier.json")
        else args.output
    )
    cfg = FoundationBackedBridgeFrontierConfig(
        ...
        max_generation_attempts=1_000 if args.smoke else args.max_generation_attempts,
    ...
    result = run_foundation_backed_bridge_frontier(config=cfg)
