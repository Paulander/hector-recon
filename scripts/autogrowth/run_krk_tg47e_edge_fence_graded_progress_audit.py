#!/usr/bin/env python3
"""Run TG47e graded edge/fence progress audit."""
...
DEFAULT_OUTPUT_DIR = "reports/autogrowth/clean_slate_krk/tg47e_edge_fence_graded_progress_audit"
...
def main() -> None:
    ...
    if not args.fresh_extension:
        raise SystemExit("TG47e requires --fresh-extension")

    output_dir = args.output_dir
    config = CleanEdgeFenceStageConfig(
        checkpoint_name="TG47e_edge_fence_graded_progress_audit",
        schema_version="krk_tg47e_edge_fence_graded_progress_audit.v0",
        progress_schema_version="krk_tg47e_edge_fence_graded_progress_audit_progress.v0",
        run_scale_label=args.run_scale_label or _run_scale_label(args),
        output_dir=output_dir,
        output_path=f"{output_dir}/krk_tg47e_edge_fence_graded_progress_audit.json",
        progress_path=f"{output_dir}/krk_tg47e_edge_fence_graded_progress_audit_progress.json",
        markdown_path=f"{output_dir}/krk_tg47e_edge_fence_graded_progress_audit.md",
        train_trace_path=f"{output_dir}/pools/tg47e_train_traces.jsonl.gz",
        eval_trace_path=f"{output_dir}/pools/tg47e_eval_traces.jsonl.gz",
        failure_pool_path=f"{output_dir}/pools/tg47e_failure_pool.jsonl.gz",
        online_trace_path=f"{output_dir}/pools/tg47e_online_episodes.jsonl.gz",
        m4_audit_log_path=f"{output_dir}/pools/tg47e_m4_audit.jsonl.gz",
        graph_summary_path=f"{output_dir}/pools/tg47e_graph_summary.json",
        promoted_edge_fence_artifact_path=f"{output_dir}/promoted_tg47e_edge_fence.json",
        seed=args.seed,
