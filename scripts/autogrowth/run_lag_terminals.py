#!/usr/bin/env python3
"""Run TG19 LAG terminal checkpoint."""
...
from recon_lite_chess.autogrowth import LagTerminalConfig, run_lag_terminal_experiment
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--output",
        type=Path,
        default=Path("reports/autogrowth/krk_autogrowth_tg19_lag_terminals.json"),
    )
...
def main() -> int:
    args = parse_args()
    default_output = Path("reports/autogrowth/krk_autogrowth_tg19_lag_terminals.json")
    output = (
        Path("reports/autogrowth/krk_autogrowth_tg19_lag_terminals_smoke.json")
    ...
    lag = payload["arms"]["real_fragment_chain_lag"][primary]
    no_lag = payload["arms"]["real_fragment_chain_no_lag"][primary]
    ...
    print(
        ...
        f"training_lag_triggers={payload['arms']['training_lag_fragment_chain']['lag_trigger_count']} "
        f"training_m3={payload['arms']['training_lag_fragment_chain']['m3_update_count']}"
    )
    print(
        f"h{primary}: "
        f"baseline_mates={baseline['mates']}/{baseline['total']} "
        f"no_lag_mates={no_lag['mates']}/{no_lag['total']} "
        f"lag_mates={lag['mates']}/{lag['total']} "
        f"lag_suppressions={lag['lag_suppression_count']} "
        f"rook_loss_delta_vs_no_lag={decision['rook_loss_delta_vs_no_lag']} "
