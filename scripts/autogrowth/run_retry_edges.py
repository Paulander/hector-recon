#!/usr/bin/env python3
"""Run TG21 local retry-edge reinforcement checkpoint."""
...
from recon_lite_chess.autogrowth import RetryEdgeConfig, run_retry_edge_experiment
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--output",
        type=Path,
        default=Path("reports/autogrowth/krk_autogrowth_tg21_retry_edges.json"),
    )
...
def main() -> int:
    args = parse_args()
    default_output = Path("reports/autogrowth/krk_autogrowth_tg21_retry_edges.json")
    output = (
        Path("reports/autogrowth/krk_autogrowth_tg21_retry_edges_smoke.json")
    ...
    edge = payload["arms"]["real_fragment_chain_retry_edges"][primary]
    ...
    print(
        ...
        f"training_retries={payload['arms']['training_retry_edge_mining']['retry_success_count']} "
        f"training_m3={payload['arms']['training_retry_edge_mining']['m3_update_count']}"
    )
    print(
        f"h{primary}: "
        f"retry_mates={retry['mates']}/{retry['total']} "
        f"edge_mates={edge['mates']}/{edge['total']} "
        f"edge_bonus_hits={edge['retry_edge_bonus_hit_count']} "
        f"completion_delta_vs_retry={decision['completion_delta_vs_tg20_retry']} "
        f"rook_losses={edge['rook_losses']} "
        f"repetition_delta_vs_retry={decision['repetition_event_delta_vs_tg20_retry']}"
    )
    print(
        "decision: "
