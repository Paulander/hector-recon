#!/usr/bin/env python3
"""Run the frozen contextual coverage-by-gain expressivity ceiling."""
...
DEFAULT_OUTPUT = Path(
    ...
    "contextual_expressivity_ceiling_20260712.json"
)
SEEDS = tuple(range(20261701, 20261721))
ARMS = (
    "sparse_bounded",
    "sparse_high_gain",
    "exhaustive_bounded",
    "exhaustive_high_gain",
)
...
def _inject_exhaustive_pairs(
    policy: EpisodicCompositionPolicy,
    task: dict[str, object],
) -> int:
    inserted = 0
    target_pairs = {
        tuple(sorted((cue_id, regime_id)))
        for cue_id in task["door_cue_literals"]
        for regime_id in task["regime_ids"]
    }
    for action_id in task["door_action_ids"]:
        ...
        for members in sorted(target_pairs - mature_pairs):
            evidence = learner.pair_evidence.get(members)
            learner.candidates.append(
                CompositeCandidate(
                    members=members,
                    born_observation=learner.observation_count,
                    proposal_score=0.0,
                    support_at_proposal=(
                        evidence.support if evidence is not None else 0
                    ),
                    state="mature",
                    shadow_weight=0.0,
                    decision_observation=learner.observation_count,
                )
            )
            learner._proposed_pairs.add(members)
            inserted += 1
...
def _configure_arm(
    ...
    exhaustive = arm.startswith("exhaustive")
    high_gain = arm.endswith("high_gain")
    bounds = (-4.0, 4.0) if high_gain else (-1.0, 1.0)
    for action_id, channel in policy.channels.items():
        ...
        channel.learner.config = replace(
            ...
            prediction_min=bounds[0],
            prediction_max=bounds[1],
            max_candidates=8 if exhaustive and is_door else 4,
            max_total_proposals=64,
        )
    return _inject_exhaustive_pairs(policy, task) if exhaustive else 0
...
def main() -> int:
    ...
    for task_index, (task, phase0_policy, checkpoint_hash) in enumerate(
        ...
        arm_rows = {}
        for arm, policy in clones.items():
            inserted = _configure_arm(policy, task, arm)
            ...
            base.update({
                "inserted_exhaustive_candidate_count": inserted,
    ...
    for arm in ARMS:
        ...
        summary = {
            ...
            "full_coverage_task_count": sum(
                row["arms"][arm]["contextual_coverage"][
                    "covered_component_count"
                ] == 8
                for row in task_rows
            ),
        }
        summary["ceiling_pass"] = all((
            ...
            (
                summary["full_coverage_task_count"] == len(task_rows)
                if exhaustive
                else True
            ),
        ))
    ...
    max_sparse_live = max(
        ...
        for row in task_rows
        for arm in ("sparse_bounded", "sparse_high_gain")
        for channel in row["arms"][arm]["channels"].values()
    )
    max_exhaustive_door_live = max(
        channel["final_live_candidate_count"]
        for row in task_rows
        for arm in ("exhaustive_bounded", "exhaustive_high_gain")
    ...
    invariants = {
        ...
        "maximum_exhaustive_door_live_candidates_observed": (
            max_exhaustive_door_live
        ),
    ...
    invariants_pass = all((
        ...
        max_total <= 64,
        max_sparse_live <= 4,
        max_exhaustive_door_live <= 8,
        mismatch == 0,
        trial_edges == 0,
    ))
    passing_arms = [
        arm for arm in ARMS if arm_summaries[arm]["ceiling_pass"]
    ]
    payload = {
        "schema_version": "recon_contextual_expressivity_ceiling_raw.v1",
        "status": "complete",
        ...
        "laboratory_ceiling_only": True,
        "source_commit": git_commit(repo_root),
        "frozen_contract": (
            "docs/autogrowth/"
            "GENERIC_CORE_CONTEXTUAL_EXPRESSIVITY_CEILING_WORK_PACKAGE_20260712.md"
        ),
