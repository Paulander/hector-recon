#!/usr/bin/env python3
"""Probe KRK strategy-sequence dataset v2 quality without enabling selection."""
...
OUT_JSON = Path("reports/strategy_arbitration/krk_strategy_sequence_dataset_v2_quality_probe.json")
OUT_MD = Path("reports/strategy_arbitration/krk_strategy_sequence_dataset_v2_quality_probe.md")
...
def build_payload(dataset: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    protected_rows = [row for row in rows if not row.get("stage7_challenge_row")]
    ...
    generator_rows = [
        row for row in rows if row.get("usable_for_candidate_generation_training_v2")
    ]
    selector_rows = [row for row in rows if row.get("usable_for_selector_training_v2")]
    quality_checks = {
        "stage7_excluded_from_readiness": summary.get("stage7_readiness_training_row_count") == 0,
        "selector_rows_absent_without_ownership_labels": len(selector_rows) == 0,
        "candidate_generation_rows_present": len(generator_rows) > 0,
        "trace_feature_channel_present": len(trace_rows) > 0,
    ...
    if not selector_rows:
        selector_blockers.append("no_explicit_ownership_selector_rows")
    if len(trace_rows) < 12:
        selector_blockers.append("runtime_trace_feature_channel_small")
    if any(row.get("stage7_challenge_row") for row in stage7_rows):
        selector_blockers.append("stage7_present_only_as_heldout_challenge")
    status = (
        "strategy_sequence_dataset_v2_quality_candidate_generation_ready_selector_blocked"
        if all(quality_checks.values())
        else "strategy_sequence_dataset_v2_quality_blocked"
    )
    return {
        "schema_version": "krk_strategy_sequence_dataset_v2_quality_probe.v1",
        ...
        "summary": {
            ...
            "stage7_challenge_row_count": len(stage7_rows),
        ...
        "interpretation": {
            "candidate_generation_dataset_usable": len(generator_rows) > 0,
            "selector_dataset_usable": False,
            "reason": (
                "V2 separates evidence channels and has protected positive-capacity "
                "rows for candidate-generation recall, but it intentionally has no "
                "selector rows because ownership labels are not recovered."
            ),
        },
        "decision": {
            "status": status,
            ...
            "recommended_next_step": (
                ...
                else "fix_dataset_v2_channel_semantics"
            ),
        },
    }
...
def write_markdown(payload: dict[str, Any]) -> None:
    ...
    lines = [
        ...
        f"- protected_row_count: {summary['protected_row_count']}",
